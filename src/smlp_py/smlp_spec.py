...
class SmlpSpec:
    def __init__(self):
        ...
        self.spec_params_dict = {
            ...
            "delta_relative": {
                "abbr": "delta_rel",
                "default": self._DEF_DELTA_RELATIVE,
                "type": float,
            ...
            "delta_absolute": {
                "abbr": "delta_abs",
                "default": self._DEF_DELTA_ABSOLUTE,
                "type": float,
                "help": "exclude (1+DELTA)*radius region for non-grid components "
            ...
            "radius_relative": {
                "abbr": "rad_rel",
                "default": self._DEF_RADIUS_RELATIVE,
                "type": float,
                ...
                + "radius value specified in the spec file [default: {}]".format(
                    str(self._DEF_RADIUS_RELATIVE)
                ),
            },
            "radius_absolute": {
                "abbr": "rad_abs",
                ...
                + "radius value specified in the spec file [default: {}]".format(
                    str(self._DEF_RADIUS_ABSOLUTE)
                ),
            },
            "alpha": {
                "abbr": "alpha",
                "default": self._DEF_ALPHA,
                "type": str,
        ...
        self._SPEC_DICTIONARY_ALPHA = (
            "alpha"  # global constraints on inputs -- knobs and free inputs
        )
        self._SPEC_DICTIONARY_BETA = "beta"  # global interface costraints to be valid during optimization and tuning
        self._SPEC_DICTIONARY_ETA = (
            "eta"  # global constraints on knobs (during candidate search)
        )
    ...
    def sanity_check_spec(self):
        ...
        for var_spec in self.spec:
            for k in var_spec.keys():
                if k not in [
                    self._SPEC_VARIABLE_LABEL,
                    self._SPEC_VARIABLE_TYPE,
                    self._SPEC_VARIABLE_RANGE,
                    self._SPEC_INPUTS_BOUNDS,
                    self._SPEC_KNOBS_GRID,
                    self._SPEC_KNOBS_ABSOLUTE_RADIUS,
                    self._SPEC_KNOBS_RELATIVE_RADIUS,
                ]:
                    raise Exception(
                        "Unexpected variable specification field "
                        + '"{}"'.format(str(k))
                    )
            # print('var_spec', var_spec)
            if self._SPEC_VARIABLE_LABEL not in var_spec.keys():
                raise Exception(
                    "A variable does not have the label (name) declared in spec file"
                )
            if self._SPEC_VARIABLE_TYPE not in var_spec.keys():
                raise Exception(
                    "Variable "
                    + str(
                        var_spec[self._SPEC_VARIABLE_LABEL]
            ...
            if self._SPEC_INPUTS_BOUNDS in var_spec.keys():
                if not var_spec[self._SPEC_VARIABLE_TYPE] in [
                    self._SPEC_INPUT_TAG,
                    self._SPEC_KNOB_TAG,
                ]:
                    raise Exception(
                        "Domain intervals (bounds) are only supported for free inputs and knobs"
                    )
            ...
            if var_spec[self._SPEC_VARIABLE_TYPE] != self._SPEC_KNOB_TAG:
                ...
                if (
                    self._SPEC_KNOBS_ABSOLUTE_RADIUS in var_spec.keys()
                    or self._SPEC_KNOBS_RELATIVE_RADIUS in var_spec.keys()
                ):
                    raise Exception("THETA radius can only be defined for knobs")
                if eta_expr is not None:
                    if var_spec[self._SPEC_VARIABLE_LABEL] in get_expression_variables(
                        eta_expr
                    ):
                        self._spec_logger.error(
                            "ETA constraint "
                            + str(eta_expr)
                            + " contains a non-knob variable "
                            + str()
                            + "; aborting..."
                        )
                ...
                both_radii_defined = (
                    self._SPEC_KNOBS_ABSOLUTE_RADIUS in var_spec
                    and self._SPEC_KNOBS_RELATIVE_RADIUS in var_spec
                )
                no_radii_defined = (
                    not self._SPEC_KNOBS_ABSOLUTE_RADIUS in var_spec
                    and not self._SPEC_KNOBS_RELATIVE_RADIUS in var_spec
                )
                if both_radii_defined or no_radii_defined:
                    raise Exception(
                        "Either a relative or an absolute radius must be specified for each knop (but not both radii). "
                        + "This fails for varaible {}.".format(
                            var_spec[self._SPEC_VARIABLE_LABEL]
                        )
                    )
        # sanity check witnesses: query names in witnesses specification must be among query names
        if (
            self.get_spec_witn_dict is not None
            and self.get_spec_quer_exprs_dict is not None
        ):
            ...
            if not witn_queries.issubset(queries):
                raise Exception(
                    "Query names "
                    + str(witn_queries.difference(queries))
                    + " used for specifying witnesses are not specified among the queries "
                    + str(queries)
                )

        # sanity check knob cinfigs: assertion names in knob config specification must be among  assertion names
        if (
            self.get_spec_config_dict is not None
            and self.get_spec_asrt_exprs_dict is not None
        ):
            ...
            if not config_asserts.issubset(asserts):
                raise Exception(
                    "Assertion names "
                    + str(config_asserts.difference(asserts))
                    + " used for specifying knob configurations are not specified among the assertions "
                    + str(asserts)
                )
    ...
    def upgrade_spec(self, spec_file, source_version, target_version):
        if not os.path.isfile(spec_file):  # +'.spec'
            raise Exception(
                "Spec file " + str(spec_file) + " does not exist"
            )  #'.spec' +
        ...
        def upgrade_token(token):
            for version_vals in self._spec_tokens_dict.values():
                ...
                token_val_source = version_vals[
                    source_version
                ]  # print('token_val_source', token_val_source)
                token_val_target = version_vals[
                    target_version
                ]  # print('token_val_target', token_val_target)
        ...
        for k, v in spec_dict.items():
            ...
            if k == self._SPEC_DICTIONARY_VERSION:
                ...
                spec_new_dict[
                    self._spec_tokens_dict["SPEC_DICTIONARY_SPEC"][target_version]
                ] = vars
            else:
                spec_new_dict[k] = v
        # print('spec_new_dict', spec_new_dict); print('new soec file', spec_file+'_v{}.spec'.format('1.2'))
        with open(
            spec_file.removesuffix(".spec") + "_v{}.spec".format("1.1"), "w"
        ) as f:
            json.dump(spec_dict, f, indent="\t", cls=np_JSONEncoder)
        with open(
            spec_file.removesuffix(".spec") + "_v{}.spec".format("1.2"), "w"
        ) as f:
    ...
    def set_spec_file(self, spec_file):
        ...
        if spec_file is not None:
            ...
            print("tis I", self._SPEC_DICTIONARY_SPEC)
            # breakpoint()
            assert self._SPEC_DICTIONARY_SPEC in spec_dict.keys()
            # print(set(spec_dict.keys()))
            assert (set(spec_dict.keys())).issubset(
                {
                    self._SPEC_DICTIONARY_VERSION,
                    self._SPEC_DICTIONARY_SPEC,
                    self._SPEC_DICTIONARY_ASSERTIONS,
                    self._SPEC_DICTIONARY_OBJECTIVES,
                    self._SPEC_DICTIONARY_QUERIES,
                    self._SPEC_DICTIONARY_ALPHA,
                    self._SPEC_DICTIONARY_BETA,
                    self._SPEC_DICTIONARY_ETA,
                    self._SPEC_DICTIONARY_SYSTEM_TAG,
                    self._SPEC_DICTIONARY_WITNESSES,
                    self._SPEC_DICTIONARY_CONFIGS,
                }
            )
            self.spec_dict = spec_dict
            self.spec = spec_dict[self._SPEC_DICTIONARY_SPEC]
            self.version = spec_dict[self._SPEC_DICTIONARY_VERSION]
            self._spec_logger.info(
                "Model exploration specification:\n" + str(self.spec_dict)
            )
    ...
    def get_spec_features(self):
        return [
            var_spec[self._SPEC_VARIABLE_LABEL]
            for var_spec in self.spec
            if var_spec[self._SPEC_VARIABLE_TYPE]
            in [self._SPEC_INPUT_TAG, self._SPEC_KNOB_TAG]
        ]
    ...
    def sanity_check_verification_spec(self):
        if self.get_spec_asrt_exprs_dict is None:
            raise Exception(
                'Assertions are not specified in "verify" mode: aborting...'
            )
        if self.get_spec_beta_global_expr is not None:
            raise Exception(
                'Beta constraints are not supported in "verify" mode: aborting...'
            )
        ...
        for var_spec in self.spec:
            ...
            if self._SPEC_KNOBS_GRID in var_spec.keys():
                if len(var_spec[self._SPEC_KNOBS_GRID]) == 0:
                    raise Exception(
                        "Knob" + str(self._SPEC_VARIABLE_LABEL) + " has an empty grid"
                    )
                if len(var_spec[self._SPEC_KNOBS_GRID]) > 1:
                    non_constant_knobs.append(var_spec[self._SPEC_VARIABLE_LABEL])
                else:
                    witness[var_spec[self._SPEC_VARIABLE_LABEL]] = var_spec[
                        self._SPEC_KNOBS_GRID
                    ][0]
                continue
            elif self._SPEC_INPUTS_BOUNDS in var_spec.keys():
                if (
                    var_spec[self._SPEC_INPUTS_BOUNDS][0]
                    != var_spec[self._SPEC_INPUTS_BOUNDS][1]
                ):
                    non_constant_knobs.append(var_spec[self._SPEC_VARIABLE_LABEL])
                else:
        ...
        if len(non_constant_knobs) > 0:
            raise Exception(
                "Knobs "
                + str(non_constant_knobs)
                + ' are not assigned constant values as part of specification, in "verify" mode: aborting...'
            )
        if len(knobs_without_range_and_grid) > 0:
            raise Exception(
                "Knobs "
                + str(knobs_without_range_and_grid)
                + ' have neither ranges nor grids specified, in "varify" mode: aborting...'
            )
        return witness
    ...
    def sanity_check_certification_spec(self):
        ...
        for var_spec in self.spec:
            if (
                not var_spec[self._SPEC_VARIABLE_LABEL]
                in self.get_spec_knobs + self.get_spec_inputs
            ):
                continue
            # print('knob spec', var_spec)
            if self._SPEC_KNOBS_GRID in var_spec.keys():
                ...
                if len(var_spec[self._SPEC_KNOBS_GRID]) > 1:
                    non_constant_knobs_inputs.append(
                        var_spec[self._SPEC_VARIABLE_LABEL]
                    )
                else:
                    witness[var_spec[self._SPEC_VARIABLE_LABEL]] = var_spec[
        ...
        if len(non_constant_knobs_inputs) > 0:
            raise Exception(
                "Knobs/Inputs "
                + str(non_constant_knobs_inputs)
                + ' are not assigned constant values as part of specification, in "certify" mode: aborting...'
            )
        if len(knobs_without_range_and_grid) > 0:
            raise Exception(
                "Knobs "
                + str(knobs_without_range_and_grid)
                + ' have neither ranges nor grids specified, in "certify" mode: aborting...'
            )
        return witness
    ...
    def get_cmdl_objectives(
        self, arg_objv_names, arg_objv_exprs, resp_names, cmdl_cond_sep
    ):
    ...
    def get_spec_quer_exprs_dict(self):
        ...
        if self._SPEC_DICTIONARY_QUERIES in self.spec_dict.keys():
            self._spec_quer_exprs = list(
                self.spec_dict[self._SPEC_DICTIONARY_QUERIES].values()
            )
    ...
    def get_spec_objv_exprs_dict(self):
        ...
        if self._SPEC_DICTIONARY_OBJECTIVES in self.spec_dict.keys():
            self._spec_objv_exprs = list(
                self.spec_dict[self._SPEC_DICTIONARY_OBJECTIVES].values()
            )
    ...
    def get_spec_component_exprs(
        ...
        # assertions
        asrt_expr_dict, asrt_names, asrt_exprs = self.get_cmdl_assertions(
            asrt_names_cmdl, asrt_exprs_cmdl, cmdl_cond_sep
        )
        asrt_expr_dict = (
            self.get_spec_asrt_exprs_dict
        )  # print('asrt_expr_dict', asrt_expr_dict)
        asrt_names = list(asrt_expr_dict.keys()) if asrt_expr_dict is not None else None
        asrt_exprs = (
            list(asrt_expr_dict.values()) if asrt_expr_dict is not None else None
        )

        # queries
        quer_expr_dict, query_names, query_exprs = self.get_cmdl_queries(
            quer_names_cmdl, quer_exprs_cmdl, cmdl_cond_sep
        )
        # print('quer_expr_dict', quer_expr_dict); print('query_names, query_exprs', query_names, query_exprs)
        quer_expr_dict = (
            self.get_spec_quer_exprs_dict
        )  # print('quer_expr_dict 2', quer_expr_dict)
        quer_names = list(quer_expr_dict.keys()) if quer_expr_dict is not None else None
        quer_exprs = (
            list(quer_expr_dict.values()) if quer_expr_dict is not None else None
        )

        # optimization objectives
        objv_expr_dict, objv_names, objv_exprs = self.get_cmdl_objectives(
            objv_names_cmdl, objv_exprs_cmdl, resp_names, cmdl_cond_sep
        )
        objv_expr_dict = (
            self.get_spec_objv_exprs_dict
        )  # print('objv_expr_dict', objv_expr_dict)
        objv_names = list(objv_expr_dict.keys()) if objv_expr_dict is not None else None
        objv_exprs = (
            list(objv_expr_dict.values()) if objv_expr_dict is not None else None
        )
    ...
    def get_spec_eta_grids_dict(self):
        ...
        for var_spec in self.spec:
            ...
            if self._SPEC_KNOBS_GRID not in var_spec.keys():
                continue
            eta_dict[var_spec[self._SPEC_VARIABLE_LABEL]] = var_spec[
                self._SPEC_KNOBS_GRID
            ]
    ...
    def get_spec_theta_radii_dict(self):
        ...
        for var_spec in self.spec:
            ...
            if (
                self.radius_relative is not None
                and self._SPEC_KNOBS_RELATIVE_RADIUS in var_spec.keys()
            ):
                ...
                continue
            if (
                self.radius_absolute is not None
                and self._SPEC_KNOBS_ABSOLUTE_RADIUS in var_spec.keys()
            ):
                theta_dict[var_spec[self._SPEC_VARIABLE_LABEL]] = {
            ...
            if (
                self._SPEC_KNOBS_ABSOLUTE_RADIUS not in var_spec.keys()
                and self._SPEC_KNOBS_RELATIVE_RADIUS not in var_spec.keys()
            ):
                continue
            if (
                self._SPEC_KNOBS_ABSOLUTE_RADIUS in var_spec.keys()
                ...
                raise Exception(
                    "Both absolute and relative radii are specified for variable "
                    + str(var_spec)
                )
            assert (
                self._SPEC_KNOBS_ABSOLUTE_RADIUS in var_spec.keys()
            ...
            if self._SPEC_KNOBS_ABSOLUTE_RADIUS in var_spec.keys():
                theta_dict[var_spec[self._SPEC_VARIABLE_LABEL]] = {
                    self._SPEC_KNOBS_ABSOLUTE_RADIUS: var_spec[
                        self._SPEC_KNOBS_ABSOLUTE_RADIUS
                    ],
                    self._SPEC_KNOBS_RELATIVE_RADIUS: None,
                }
            if self._SPEC_KNOBS_RELATIVE_RADIUS in var_spec.keys():
                theta_dict[var_spec[self._SPEC_VARIABLE_LABEL]] = {
                    self._SPEC_KNOBS_ABSOLUTE_RADIUS: None,
                    self._SPEC_KNOBS_RELATIVE_RADIUS: var_spec[
                        self._SPEC_KNOBS_RELATIVE_RADIUS
                    ],
                }
    ...
    def get_spec_domain_dict(self):
        ...
        for var_spec in self.spec:
            ...
            if self._SPEC_INPUTS_BOUNDS in var_spec.keys():
                assert var_spec[self._SPEC_VARIABLE_TYPE] in [
                    self._SPEC_INPUT_TAG,
                    self._SPEC_KNOB_TAG,
                ]
            ...
            #    continue
            var_bounds = (
                var_spec[self._SPEC_INPUTS_BOUNDS]
                if self._SPEC_INPUTS_BOUNDS in var_spec.keys()
                else None
            )
            ...
            if not var_range in [self._SPEC_RANGE_INTEGER, self._SPEC_RANGE_REAL]:
                raise Exception(
                    "Unsupported variable range (type) "
                    + var_spec[self._SPEC_VARIABLE_RANGE]
                )
            # self._domain_dict[var_spec['label']] = {'range': var_range, 'interval': var_bounds}
            self._domain_dict[var_spec[self._SPEC_VARIABLE_LABEL]] = {
                self._SPEC_DOMAIN_TYPE_TAG: var_range,
                self._SPEC_DOMAIN_INTERVAL_TAG: var_bounds,
            }
        ...
        for var_spec in self.spec:
            ...
            if self._SPEC_INPUTS_BOUNDS in var_spec.keys():
                assert (
                    ...
                    or var_spec[self._SPEC_VARIABLE_TYPE] == self._SPEC_KNOB_TAG
                )
                if var_spec[self._SPEC_VARIABLE_TYPE] == self._SPEC_KNOB_TAG:
                    continue
                ...
                if mn is not None and mx is not None:
                    if mx < mn:
                        ...
                        raise Exception(
                            ...
                            + str(range_str)
                            + " of variable "
                            + str(var_spec[self._SPEC_VARIABLE_LABEL])
                            + " is not specified correcly"
                        )
    ...
    def get_spec_stability_ragion_bounds_dict(self, knob_config):
        ...
        for k, v in self.get_spec_theta_radii_dict.items():
            ...
            if v["rad-abs"] is not None:
                ...
                raise Exception(
                    "At least on of the relative or absolute radii must mot be None"
                )
            knobs_ranges[k] = [
                knob_val - rad,
                knob_val + rad,
            ]  # {'min':knob_val - rad, 'max':knob_val + rad}
    ...
    def get_spec_stability_intervals_dict(self, knob_config, include_inputs):
        ...
        if include_inputs:
            ...
            for k, v in self.get_spec_alpha_bounds_dict.items():
                input_ranges[k] = [
                    v["min"] if v["min"] is not None else (-np.inf),
        ...
        for k, v in self.get_spec_theta_radii_dict.items():
            # print('k', k, 'v', v)
            if (
                k not in knob_config
            ):  # TODO !!!! this should not happen with full assighnement to knobs
            ...
            knob_range_min = (
                knob_val - rad
            )  # print('knob_range_min before', knob_range_min)
            knob_range_max = (
                knob_val + rad
            )  # print('knob_range_max before', knob_range_max)
            ...
            if k in self.get_spec_eta_bounds_dict:
                knob_interval = self.get_spec_eta_bounds_dict[
                    k
                ]  # print('knob_interval', knob_interval)
    ...
    def split_spec(self, spec_file, knobs: list[str], splits: list[int]):
        ...
        for name in knobs:
            for variable in variables:
                if (
                    variable.get(self._SPEC_VARIABLE_LABEL) == name
                    and variable.get(self._SPEC_VARIABLE_TYPE) == self._SPEC_KNOB_TAG
                ):
                ...
                raise ValueError(
                    f"No knob variable found with label '{name}' or missing 'grid' key."
                )

        # Step 4: Split the "grid" lists into sub-grids
        sub_grids = {
            name: [grids[name][i :: splits[idx]] for i in range(splits[idx])]
            for idx, name in enumerate(knobs)
        }
        ...
        for idx, combo in enumerate(combinations):
            ...
            new_spec_data[self._SPEC_DICTIONARY_SPEC] = [
                var.copy() for var in variables
            ]
