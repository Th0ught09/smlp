...
class ModelKeras:
    def __init__(self):
        ...
        self.SMLP_KERAS_MODELS = [
            self._algo_name_local2global(m) for m in self._KERAS_MODELS
        ]
        ...
        self._DEF_OPTIMIZER = (
            "adam"  # options: 'rmsprop', 'adam', 'sgd', 'adagrad', 'nadam'
        )
        ...
        * Custom Training Loops: If you implement custom training loops, you can use metrics to monitor additional 
        ...
        self._TUNER_EXECUTIONS_PER_TRIAL = (
            1  # Number of models to train for each trial, for RandomSearch()
        )
        ...
        self._keras_hparam_default_dict = {
            "layers": {
                ...
                + "(the input layer has one node per input) [default: {}]".format(
                    self._DEF_LAYERS_SPEC
                ),
            },
            "epochs": {
                "abbr": "epochs",
                "default": self._DEF_EPOCHS,
                "type": int,
                "help": "epochs for NN [default: {}]".format(self._DEF_EPOCHS),
            },
            "batch_size": {
                "abbr": "batch",
                "default": self._DEF_BATCH_SIZE,
                "type": int,
                "help": "batch_size for NN [default: not exposed]".format(
                    self._DEF_BATCH_SIZE
                ),
            },
            "optimizer": {
                "abbr": "optimizer",
                "default": self._DEF_OPTIMIZER,
                "type": str,
                "help": "optimizer for NN [default: {}]".format(self._DEF_OPTIMIZER),
            },
            "learning_rate": {
                "abbr": "learning_rate",
                "default": self._DEF_LEARNING_RATE,
                "type": float,
                "help": "optimizer for NN [default: {}]".format(
                    self._DEF_LEARNING_RATE
                ),
            },
            "loss_function": {
                "abbr": "loss",
                "default": self._DEF_LOSS,
                "type": str,
            ...
            "metrics": {
                "abbr": "metrics",
                "default": self._DEF_METRICS,
                "type": str_to_str_list,
            ...
            "hid_activation": {
                "abbr": "hid_activation",
                "default": self._HID_ACTIVATION,
                "type": str,
                "help": "hidden layer activation for NN [default: {}]".format(
                    self._HID_ACTIVATION
                ),
            },
            "out_activation": {
                "abbr": "out_activation",
                "default": self._OUT_ACTIVATION,
                "type": str,
                "help": "output layer activation for NN [default: {}]".format(
                    self._OUT_ACTIVATION
                ),
            },
            "sequential_api": {
                "abbr": "seq_api",
                "default": self._SEQUENTIAL_API,
                "type": str_to_bool,
                "help": "Should sequential api be used building NN layers or should functional "
                + "api be used instead? [default: {}]".format(
                    str(self._SEQUENTIAL_API)
                ),
            },
            "weights_precision": {
                "abbr": "weights_precision",
                "default": self._WEIGHTS_PRECISION,
                "type": int,
                ...
                + "implies that weight will not be rounded [default: {}]".format(
                    self._OUT_ACTIVATION
                ),
            },
            "tuner_algo": {
                "abbr": "tuner",
                "default": self._DEF_TUNER_ALGO,
                "type": str,
            ...
            "layers_grid": {
                "abbr": "layers_grid",
                "default": self._DEF_LAYERS_SPEC_GRID,
                "type": str_to_str_list_list,
                ...
                "for a detailed description [default: {}]".format(
                    str(self._DEF_LAYERS_SPEC_GRID)
                ),
            },
            ...
            "batches_grid": {
                "abbr": "batches_grid",
                "default": self._DEF_BATCH_SIZE_GRID,
            ...
            "learning_rates_grid": {
                "abbr": "lrates_grid",
                "default": self._DEF_LEARNING_RATES_GRID,
            ...
            "loss_functions_grid": {
                "abbr": "losses_grid",
                "default": self._DEF_LOSS_FUNCTIONS_GRID,
                "type": str_to_str_list,
    ...
    def _param_dict_with_algo_name(self, param_dict, algo):
        ...
        for k, v in param_dict.items():
            v_updated = v.copy()
            v_updated["abbr"] = self._hparam_name_local_to_global(
                v["abbr"], algo
            )  # algo + '_' + v['abbr']
            # print('updated abbrv', v_updated['abbr'])
            # print('updated key', self._hparam_name_local_to_global(k, algo))
            result_dict[self._hparam_name_local_to_global(k, algo)] = (
                v_updated  # algo + '_' + k
            )
        # raise Exception('tmp')
        return result_dict
    ...
    def get_keras_hparam_default_dict(self):
        nn_keras_hyperparam_dict = self._param_dict_with_algo_name(
            self._keras_hparam_default_dict, "nn"
        )
        return nn_keras_hyperparam_dict

    # initialize Keras NN model using sequential API; it does not use sample weights.
    def _nn_init_model_sequential(
        self,
        resp_names: list[str],
        input_dim: int,
        optimizer: str,
        ...
        for i, size in enumerate(layers_spec_list):
            if i == 0:
                ...
                model.add(
                    keras.layers.Dense(
                        units=size, activation=hid_activation, input_shape=(input_dim,)
                    )
                )
            else:
                self._keras_logger.info("dense layer of size " + str(size))
        ...
        if n_out == 1:
            ...
            model.add(
                keras.layers.Dense(n_out, activation=out_activation, name=resp_names[0])
            )
        else:
    ...
    def compare_model_configs_from_files(self, file1, file2):
        ...
        keys_to_ignore += [
            key for key in config2.keys() if "layers" in key
        ]  # Include keys from both configs
        ...
        differences = {}
        for key in set(config1.keys()).union(
            config2.keys()
        ):  # Union of keys from both configs
        ...
        if differences:
            ...
            print(
                "No differences found in model configurations (excluding layer topology)."
            )
    ...
    def _log_model_summary(self, model, epochs, batch_size, sample_weights, callbacks):
        ...
        self._keras_logger.info(
            "Learning rate: " + str(model.optimizer.learning_rate.numpy())
        )
        if isinstance(
            model.loss, dict
        ):  # functiona API, and NN Keras Tuner is not used
        ...
        if hasattr(model, "compiled_metrics") and False:
            compiled_metrics = (
                model.compiled_metrics._metrics
            )  # Access the private _metrics attribute
            self._keras_logger.info(
                "Metrics: " + str([m.name for m in compiled_metrics])
            )
        else:
        ...
        self._keras_logger.info(
            "Callbacks: " + str([str(type(clb)) for clb in callbacks])
        )
    ...
    def _nn_train(
        ...
        if sequential_api:  # SEQUENTIAL_MODEL
            if sample_weights_dict is not None:
                ...
                sample_weights_vect = np.array(
                    list(sample_weights_df.agg("mean", axis=1))
                )
            else:
    ...
    def _report_training_regression(
        self, history, metrics, epochs, interactive, resp_names, out_prefix=None
    ):
        ...
        def plot_error_convergence(
            acc: list[float], val_acc: list[float], resp_name: str, metric: str
        ):
            ...
            plt.legend(loc="best")
            plt.title(
                "Response {} Training and Validation {}".format(resp_name, metric)
            )
            plt.xlabel("epochs")
        ...
        for metric in metrics:
            ...
            history_for_all_responses = (
                metric.name in history.history
                and "val_" + metric.name in history.history
                and "loss" in history.history
                and "val_loss" in history.history
            )
    ...
    def model_size_accuracy_tradeoff_objective(
        self, accuracy, model_size, accuracy_tradeoff, size_tradeoff
    ):
        """
    ...
    def tuner_objective(self, trial):
        ...
        score = self.model_size_accuracy_tradeoff_objective(
            val_accuracy, model_size, accuracy_tradeoff, size_tradeoff
        )
    ...
    def build_model(
        ...
        topology_sizes = dict(
            [
                (layers_str, [e * input_dim for e in str_to_int_list(layers_str)])
                for layers_str in layers_grid
            ]
        )
        ...
        lr = (
            hp.Choice("learning_rate", values=lrates_grid)
            if lrates_grid is not None
            else 0.001
        )

        # Choose the loss function
        loss = (
            hp.Choice("loss_function", values=losses_grid)
            if losses_grid is not None
            else self._DEF_LOSS
        )

        optimizer = keras.optimizers.Adam(learning_rate=lr)
        if sequential_api:  # SEQUENTIAL_MODEL
            model = self._nn_init_model_sequential(
                resp_names,
                input_dim,
                optimizer,
                hid_activation,
            ...
            model = self._nn_init_model_functional(
                resp_names,
                input_dim,
                optimizer,
                hid_activation,
    ...
    def search(
        ...
        self._keras_logger.info(
            "Tuning model hyperparameters using Keras Tuner algorithm "
            + str(tuner_algo)
            + ": start"
        )
        ...
        self.tuner.search(
            ...
            callbacks=[
                keras.callbacks.EarlyStopping(
                    patience=self._TUNER_EARLY_STOPPING_PATIENCE
                )
            ],
            batch_size=self.tuner.oracle.hyperparameters.Choice(
                "batch_size", values=batches_grid
            )
            if batches_grid is not None
            else None,
        )
        ...
        for hyperparam in best_hps.space:
            self._keras_logger.info(
                f"{hyperparam.name}: {best_hps.get(hyperparam.name)}"
            )
    ...
    def train_best_model(
        ...
        best_model, history = self.get_best_model(
            X_train,
            X_test,
            y_train,
            y_test,
    ...
    def _keras_train_multi_response(
        self,
        resp_names: list[str],
        algo: str,
        X_train,
        ...
        layers_spec = self._get_parm_val(
            hparam_dict, self._hparam_name_local_to_global("layers", algo)
        )  # DEF_LAYERS_SPEC 'nn_layers'
        epochs = self._get_parm_val(
            hparam_dict, self._hparam_name_local_to_global("epochs", algo)
        )  # DEF_EPOCHS)
        batch_size = self._get_parm_val(
            hparam_dict, self._hparam_name_local_to_global("batch_size", algo)
        )  # DEF_BATCH_SIZE
        optimizer = self._get_parm_val(
            hparam_dict, self._hparam_name_local_to_global("optimizer", algo)
        )  # DEF_OPTIMIZER
        learning_rate = self._get_parm_val(
            hparam_dict, self._hparam_name_local_to_global("learning_rate", algo)
        )  # DEF_OPTIMIZER
        loss_function_str = self._get_parm_val(
            hparam_dict, self._hparam_name_local_to_global("loss_function", algo)
        )  # DEF_LOSS
        metrics_str_list = self._get_parm_val(
            hparam_dict, self._hparam_name_local_to_global("metrics", algo)
        )  # DEF_METRICS
        hid_activation = self._get_parm_val(
            hparam_dict, self._hparam_name_local_to_global("hid_activation", algo)
        )  # HID_ACTIVATION
        out_activation = self._get_parm_val(
            hparam_dict, self._hparam_name_local_to_global("out_activation", algo)
        )  # OUT_ACTIVATION
        sequential_api = self._get_parm_val(
            hparam_dict, self._hparam_name_local_to_global("sequential_api", algo)
        )  # SEQUENTIAL_API
        weights_precision = self._get_parm_val(
            hparam_dict, self._hparam_name_local_to_global("weights_precision", algo)
        )
        tuner_algo = self._get_parm_val(
            hparam_dict, self._hparam_name_local_to_global("tuner_algo", algo)
        )
        ...
        metrics = [
            self._metrics.get(metrics_str) for metrics_str in metrics_str_list
        ]  # print('metrics', metrics_str_list, metrics)
        ...
        if tuner_algo is not None:
            layers_grid = self._get_parm_val(
                hparam_dict, self._hparam_name_local_to_global("layers_grid", algo)
            )
            if layers_grid is None:
                assert layers_spec is not None
                layers_grid = [layers_spec]
            lrates_grid = self._get_parm_val(
                hparam_dict,
                self._hparam_name_local_to_global("learning_rates_grid", algo),
            )
            if lrates_grid is None:
                assert learning_rate is not None
                lrates_grid = [learning_rate]
            losses_grid = self._get_parm_val(
                hparam_dict,
                self._hparam_name_local_to_global("loss_functions_grid", algo),
            )
            if lrates_grid is None:
                assert loss_function_str is not None
                lrates_grid = [loss_function_str]
            batches_grid = self._get_parm_val(
                hparam_dict, self._hparam_name_local_to_global("batches_grid", algo)
            )
            if batches_grid is None:
                assert batch_size is not None
                batches_grid = [batch_size]
            tuned_model = self.train_best_model(
                resp_names,
                algo,
                X_train,
                X_test,
        ...
        layers_spec_list = [
            float(x) for x in layers_spec.split(",")
        ]  # map(float, layers_spec.split(','));
    ...
    def keras_main(
        ...
        if model_per_response:
            ...
            for rn in resp_names:
                rn_weights_coef = (
                    {rn: weights_coef[rn]} if weights_coef is not None else weights_coef
                )
                rn_model = self._keras_train_multi_response(
                    [rn],
                    algo,
                    X_train,
                    X_test,
            ...
            model = self._keras_train_multi_response(
                resp_names,
                algo,
                X_train,
                X_test,
