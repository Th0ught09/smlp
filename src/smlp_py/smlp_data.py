...
class SmlpData:
    def __init__(self):
        ...
        self._DEF_TRAIN_UNIF = (
            0  # sampling from training data to acheive uniform distribution
        )
        self._DEF_SCALER = "min_max"  # options: 'min_max', 'max-abs'
        self._DEF_SPLIT_TEST = (
            0.2  # ratio to split training data into training and validation subsets
        )
        self._DEF_SAMPLE_WEIGHTS_INTERCEPT = (
            0  # intercept a of the power function a+bx**c that computes the
        )
        # sample weights based on response values on these samples
        self._DEF_SAMPLE_WEIGHTS_COEFFICIENT = (
            0  # coefficient b of the power function a+bx**c that computes the
        )
        # sample weights based on response values on these samples
        self._DEF_SAMPLE_WEIGHTS_EXPONENT = (
            0  # degree/exponent c of the power function a+bx**c that computes the
        )
        # sample weights based on response values on these samples
        self._RESPONSE_TO_BOOL = (
            None  # list of conditions per response to convert numeric responses
        )
        ...
        self._CONDITION_SEPARATOR = (
            ";"  # separator used in self._RESPONSE_TO_BOOL to seperate conditions
        )
        self._DEF_SCALE_FEATURES = True
        self._DEF_SCALE_RESPONSES = True
        self._DEF_IMPUTE_RESPONSES = False
        self._DEF_RESPONSE_PLOTS = (
            True  # should response values distribution plots be genrated?
        )
        ...
        self.data_params_dict = {
            ...
            "keep_features": {
                "abbr": "keep_feat",
                "default": self._DEF_KEEP_FEATURES,
                "type": str,
                ...
                + "cannot drop these input features [default {}]".format(
                    str(self._DEF_KEEP_FEATURES)
                ),
            },
            "new_data": {
                "abbr": "new_data",
            ...
            "impute_responses": {
                ...
                "has a missing value [default: "
                + str(self._DEF_IMPUTE_RESPONSES)
                + "]",
            },
            "split_test": {
                "abbr": "split",
                "default": self._DEF_SPLIT_TEST,
                "type": float,
                ...
                + " both for training and testing [default: {}]".format(
                    str(self._DEF_SPLIT_TEST)
                ),
            },
            "train_random_n": {
                "abbr": "train_rand",
                "default": self._DEF_TRAIN_RAND,
                "type": int,
            ...
            "train_uniform_n": {
                "abbr": "train_unif",
                "default": self._DEF_TRAIN_UNIF,
                "type": int,
                "help": "Subset random n rows from training data with close to uniform "
                + "distribution to use for training [default: {}]".format(
                    str(self._DEF_TRAIN_UNIF)
                ),
            },
            "sample_weights_coef": {
                "abbr": "sw_coef",
                "default": self._DEF_SAMPLE_WEIGHTS_COEFFICIENT,
                "type": float,
                ...
                + "for each sample [default: {}]".format(
                    self._DEF_SAMPLE_WEIGHTS_COEFFICIENT
                ),
            },
            "sample_weights_exponent": {
                "abbr": "sw_exp",
                "default": self._DEF_SAMPLE_WEIGHTS_EXPONENT,
                "type": float,
            ...
            "sample_weights_intercept": {
                "abbr": "sw_int",
                "default": self._DEF_SAMPLE_WEIGHTS_INTERCEPT,
                "type": float,
            ...
            "response_to_bool": {
                "abbr": "resp2b",
                "default": self._RESPONSE_TO_BOOL,
                "type": str,
                ...
                "for the remaining samples [default: {}]".format(
                    str(self._RESPONSE_TO_BOOL)
                ),
            },
            "positive_value": {
                "abbr": "pos_val",
                "default": self.SMLP_POSITIVE_VALUE,
                "type": str,
            ...
            "negative_value": {
                "abbr": "neg_val",
                "default": self.SMLP_NEGATIVE_VALUE,
                "type": str,
            ...
            "response_plots": {
                "abbr": "resp_plots",
                "default": self._DEF_RESPONSE_PLOTS,
                "type": str_to_bool,
                ...
                + "displayed interactively during runtime [default: "
                + str(self._DEF_RESPONSE_PLOTS)
                + "]",
            },
    ...
    def _sanity_check_responses(self, df, resp_names, is_training):
        if resp_names is None:
            self._data_logger.error(
                "error: no response names were provided", file=sys.stderr
            )
        ...
        if is_training:
            ...
            if len(missing_responses) > 0:
                raise Exception(
                    "Responses "
                    + str(missing_responses)
                    + " do not occur in input data"
                )
        else:
            if len(missing_responses) > 0 and len(missing_responses) != len(resp_names):
                raise Exception(
                    "The fillowing responses "
                    + str(missing_responses)
                    + " do not occur in input data"
                )
    ...
    def _get_response_features(
        self, df, feat_names, resp_names, is_training, new_labeled
    ):
    ...
    def _drop_rows_with_na_in_responses(self, df, resp_names, data_name):
        ...
        if rows_before != rows_after:
            self._data_logger.info(
                ...
                + " rows where at least one response is NA have been dropped from "
                + str(data_name)
                + " data"
            )
        return df
    ...
    def _drop_constant_features(
        self, df: pd.DataFrame, keep_feat: list[str], data_name: str
    ):
        constant_cols = []
        # print('df\n', df)
        for col in df.columns.tolist():
            ...
            )  # print('col', col, 'unique', unique_vals)
        ...
        if len(constant_cols_to_drop) > 0:
            self._data_logger.info(
                "The following constant features have been droped from "
                + str(data_name)
                + " data:"
            )
    ...
    def _apply_transformation_to_columns(self, df, resp_names, transformation):
        ...
        if transformation is None:
            return df
        transform_lambda = eval(
            "lambda x: " + transformation.format("x")
        )  # print('transform_lambda', transform_lambda)
    ...
    def _preprocess_responses(
        ...
        for i, resp_name in enumerate(resp_names):
            ...
            assert (
                resp_type != bool
            )  # columns of boolean type were already converted to object type
            assert resp_type in [
                int,
                float,
                object,
            ]  # the only expected / suported column types
            ...
            pos_val = cast_type(
                pos_value, resp_val_type
            )  # print('pos_value', pos_value, type(pos_value))
            neg_val = cast_type(
                neg_value, resp_val_type
            )  # print('neg_value', neg_value, type(neg_value))
            ...
            if set(resp_unique_vals) == {STAT_NEGATIVE_VALUE, STAT_POSITIVE_VALUE}:
                if pos_val == 0 and neg_val == 1:
                    ...
                    resp_df[resp_name] = resp_df.replace(
                        {
                            resp_name: {
                                pos_val: STAT_POSITIVE_VALUE,
                                neg_val: STAT_NEGATIVE_VALUE,
                            }
                        }
                    )
                elif not (
                    pos_val == STAT_POSITIVE_VALUE and neg_val == STAT_NEGATIVE_VALUE
                ):
                    raise Exception(
                        "Options positive_value and negative value might not be specified correctly"
                    )
                ...
                if {neg_val, pos_val} == set(resp_unique_vals):
                    ...
                    resp_df[resp_name].replace(
                        {pos_val: STAT_POSITIVE_VALUE, neg_val: STAT_NEGATIVE_VALUE},
                        inplace=True,
                    )
                    ...
                    raise Exception(
                        "Response "
                        + str(resp_name)
                        + " has two string (categorical) values "
                        + str(resp_unique_vals)
                        + " not matching set ({neg}, {pos})".format(
                            neg=STAT_NEGATIVE_VALUE, pos=STAT_POSITIVE_VALUE
                        )
                        + "; Please use options --positive_value and "
                ...
                if resp_type == object:
                    if len(resp_unique_vals) > 2:
                        raise Exception(
                            "Response "
                            + str(resp_name)
                            + "has more then two string (categorical) "
        ...
        if resp_type in [int, float] and resp_to_bool is not None:
            ...
            for resp_name in resp_names:
                df_resp_cond = df_resp_cond.replace(
                    "{}".format(resp_name), "resp_df[resp_name]"
                )
            # print('df_resp_cond', df_resp_cond)

            resp_conds = df_resp_cond.split(
                self._CONDITION_SEPARATOR
            )  # print('resp_conds', resp_conds)
            for resp_name, resp_cond in zip(resp_names, resp_conds):
                ...
                resp_new = eval(
                    resp_cond, {}, {"resp_name": resp_name, "resp_df": resp_df}
                )
    ...
    def _save_data_bounds(
        ...
        if mm_scaler_feat is None and mm_scaler_resp is None:
            ...
            feat_bounds_dict = (
                {
                    col: {
                        "min": mm_scaler_feat.data_min_[i],
                        "max": mm_scaler_feat.data_max_[i],
                    }
                    for i, col in enumerate(feat_names)
                }
                if mm_scaler_feat is not None
                else {}
            )
            resp_bounds_dict = (
                {
                    col: {
                        "min": mm_scaler_resp.data_min_[i],
                        "max": mm_scaler_resp.data_max_[i],
                    }
                    for i, col in enumerate(resp_names)
                }
                if mm_scaler_resp is not None
                else {}
            )
        ...
        if data_bounds_dict != {}:
            self._data_logger.info(
                "Saving data bounds into file:" + str(data_bounds_file)
            )
    ...
    def _save_data_scaler(
        self,
        scale_feat: bool,
        scale_resp: bool,
        mm_scaler_feat,
        mm_scaler_resp,
        features_scaler_file: str,
    ...
    def _sample_first_random_unifirm(self, X, y, first_n, random_n, uniform_n):
        ...
        if first_n >= 1:
            ...
            for y_i in y.columns:
                uniform_n_i = round(
                    uniform_n / y.shape[1]
                )  # print('uniform_n_i', uniform_n_i)
                uniform_n_y_i = np.random.uniform(
                    low=y[y_i].min(), high=y[y_i].max(), size=uniform_n_i
                )
    ...
    def preprocess_data(
        ...
        feat_names: list[str],
        resp_names: list[str],
        feat_names_dict: dict,
        keep_feat: list[str],
        impute_resp: bool,
        data_version_str: str,
        pos_value: int,
        neg_value: int,
        resp_map: str,
        resp_to_bool: str,
        ...
        keep_feat = list_unique_unordered(
            keep_feat + self._specInst.get_spec_constraint_vars()
        )
        ...
        if feat_names is None and is_training:
            ...
            feat_names = lists_union_order_preserving_without_duplicates(
                list(feat_names_dict.values())
            )
        ...
        if is_training:
            if not impute_resp:
                data = self._drop_rows_with_na_in_responses(
                    data, resp_names, "training"
                )  # print('data 1\n', data)
            data, constant_feat = self._drop_constant_features(
                data, keep_feat, "training"
            )  # print('constant_feat', constant_feat); print('data 2\n', data)
            resp_names = [
                rn for rn in resp_names if not rn in constant_feat
            ]  # print('resp_names', resp_names)
            feat_names = [
                fn for fn in feat_names if not fn in constant_feat
        ...
        self._data_logger.info(
            data_version_str + " data after imputing missing values\n" + str(data)
        )
        ...
        X, y = self._get_response_features(
            data, feat_names, resp_names, is_training, new_labeled
        )
        if is_training or new_labeled:
            y = self._preprocess_responses(
                y, pos_value, neg_value, resp_map, resp_to_bool, is_training
            )
            self._data_logger.info(
                data_version_str
                + " data after processing responses\n"
    ...
    def _scale_data(
        ...
        if scale_feat and scale_resp:
            ...
            self._data_logger.info(
                data_version_str
                + " data after scaling (normalizing) features\n"
                + str(pd.concat([X, y], axis=1))
            )
        elif scale_resp and y is not None:
            self._data_logger.info(
                data_version_str
                + " data after scaling (normalizing) responses\n"
                + str(pd.concat([X, y], axis=1))
            )
        ...
        return (
            ...
            mm_scaler_feat,
            mm_scaler_resp,
        )  # feat_names, resp_names, model_features_dict
    ...
    def _split_data_for_training(
        self, X, y, split_test, train_first_n, train_random_n, train_uniform_n
    ):
        if split_test == 1:
            X_train, X_test, y_train, y_test = (X, X, y, y)
        else:
            X_train, X_test, y_train, y_test = train_test_split(
                X, y, test_size=split_test, shuffle=True, random_state=17
            )
        ...
        X_train, y_train = self._sample_first_random_unifirm(
            X_train, y_train, train_first_n, train_random_n, train_uniform_n
        )
    ...
    def _prepare_data_for_modeling(
        ...
        self._data_logger.info(
            "Preparing " + data_version_str + " data for modeling: start"
        )
        ...
        X, y, feat_names, resp_names, model_features_dict = self.preprocess_data(
            data_file,
            feat_names,
            resp_names,
            model_features_dict,
        ...
        if try_correlations:  # use test 46
            ...
            instCorr.mean_encode_categorical_feature_to_ordered(
                X["WAFER_ID"], y["PF"], reuse_levels=True
            )
            instCorr.mean_encode_categorical_feature_to_numeric(X["WAFER_ID"], y["PF"])
            instCorr.resp_mi_corr_feat(
                X,
                y[resp_names[0]],
                resp_names[0],
                float,
        ...
        if is_training:
            ...
            for rn in resp_names:
                mrmr_feat, _ = self._mrmrInst.smlp_mrmr(
                    X, y[rn], mrmr_features_n
                )  # print('mrmr_feat', mrmr_feat)
                model_feat = [
                    ft for ft in feat_names if (ft in mrmr_feat or ft in keep_feat)
                ]  # print(model_feat);
            ...
            feat_names = [
                ...
                if ft
                in lists_union_order_preserving_without_duplicates(
        ...
        X, y, mm_scaler_feat, mm_scaler_resp = self._scale_data(
            X,
            y,
            scale_features,
            scale_responses,
        ...
        # hashing https://engineeringfordatascience.com/posts/ml_repeatable_splitting_using_hashing/
        if is_training:
            X_train, y_train, X_test, y_test = self._split_data_for_training(
                X, y, split_test, train_first_n, train_random_n, train_uniform_n
            )
            res = (
                X,
                y,
        ...
        self._data_logger.info(
            "Preparing " + data_version_str + " data for modeling: end"
        )
        return res
    ...
    def process_data(
        ...
        if data_file is not None:
            ...
            (
                ...
                levels_dict,
                model_features_dict,
            ) = self._prepare_data_for_modeling(
                data_file,
                True,
                split_test,
                feat_names,
            ...
            self._save_model_features(
                resp_names, feat_names, self.model_features_dict_file
            )
            self._save_data_scaler(
                scale_features,
                scale_responses,
                mm_scaler_feat,
                mm_scaler_resp,
                self.features_scaler_file,
                self.responses_scaler_file,
            )
            self._save_data_bounds(
                pd.concat([X, y], axis=1),
                feat_names,
                resp_names,
                self.data_bounds_file,
                mm_scaler_feat,
                mm_scaler_resp,
            )
        else:
            ...
            mm_scaler_feat, mm_scaler_resp = self._load_data_scaler(
                scale_features,
                scale_responses,
                self.features_scaler_file,
                self.responses_scaler_file,
            ...
            model_features_dict = self._load_model_features(
                self.model_features_dict_file
            )
            feat_names = lists_union_order_preserving_without_duplicates(
                list(model_features_dict.values())
            )
        ...
        if not data_file is None and not new_data_file is None:
            common_features = list_intersection(
                X.columns.tolist(), X_new.columns.tolist()
            )
            X = X[common_features]
