...
class SmlpModels:
    def __init__(self):
        ...
        self._model_params_common_dict = {
            ...
            "save_model_rerun_configuration": {
                "abbr": "save_model_config",
                "default": self._DEF_SAVE_MODEL_CONFIG,
                "type": str_to_bool,
                "help": "Should a config file enabling to re-run a saved model be written out? "
                + "[default: "
                + str(self._DEF_SAVE_MODEL_CONFIG)
                + "]",
            },
            "model_per_response": {
                "abbr": "model_per_response",
                "default": self._MODEL_PER_RESPONSE,
                "type": str_to_bool,
                ...
                + "[default: "
                + str(self._MODEL_PER_RESPONSE)
                + "]",
            },
            "prediction_plots": {
                "abbr": "pred_plots",
                "default": self._DEF_PREDICTION_PLOTS,
                "type": str_to_bool,
                ...
                + "[default: "
                + str(self._DEF_PREDICTION_PLOTS)
                + "]",
            },
        }
    ...
    def prediction_precisions_filename(self, data_version):
        assert self.report_file_prefix is not None
        return (
            self.report_file_prefix + "_" + data_version + "_prediction_precisions.csv"
        )
    ...
    def _load_model_rerun_config(self):
        self._model_logger.info(
            "Seving model rerun configuration in file "
    ...
    def model_features_sanity_check(
        self,
        ...
        if model_features_dict is not None:
            for resp, feats in model_features_dict.items():
                ...
                if train_df is not None:
                    feats_ordered = [
                        ft for ft in train_df.columns.tolist() if ft in feats
                    ]
                    assert feats == feats_ordered
                if test_df is not None:
                    feats_ordered = [
                        ft for ft in test_df.columns.tolist() if ft in feats
                    ]
                    assert feats == feats_ordered
                if feat_df is not None:
                    feats_ordered = [
                        ft for ft in feat_df.columns.tolist() if ft in feats
                    ]
                    assert feats == feats_ordered
        ...
        if not isinstance(pred[0], list):
            ...
            if not resp is None:
                if len(pred) != resp.shape[0]:
                    raise Exception(
                        "Implementation error in function pred_results_to_df"
                    )
            pred_ind = (
                resp.index if not resp is None else range(len(pred))
            )  # print('pred_ind', pred_ind)
            predictions_df = pd.DataFrame(
                pred, index=pred_ind, columns=predictions_colnames
            )
        else:
            # we have multiple response prediction
            pred_ind = (
                resp.index if not resp is None else range(pred.shape[0])
            )  # print('pred_ind', pred_ind)
            predictions_df = pd.DataFrame(
                pred, index=pred_ind, columns=predictions_colnames
            )
    ...
    def _sample_weights_per_response_vals(self, resp_vals, sw_coef, sw_exp, sw_int):
        ...
        if old:
            ...
            mx = resp_vals.max()
            sw = [
                sw_int + sw_coef * ((v - mn) / (mx - mn)) ** sw_exp for v in resp_vals
            ]
    ...
    def _compute_sample_weights_dict(self, y_train, sw_coef, sw_exp, sw_int):
        ...
        for resp in y_train.columns.tolist():
            ...
            sw = self._sample_weights_per_response_vals(
                sw, sw_coef, sw_exp, sw_int
            )  # print('sw', len(sw))
            sw_dict[resp] = sw
    ...
    def _compute_sample_weights_vect(self, y_train, sw_coef, sw_exp, sw_int):
        ...
        return self._sample_weights_per_response_vals(
            resp_vals, sw_coef, sw_exp, sw_int
        )
    ...
    def _report_prediction_results(
        self,
        algo: str,
        resp_names: list[str],
        ...
        self._model_logger.info(
            "Saving predictions summary into file: \n"
            + str(self.predictions_summary_filename(data_version))
        )
        predictions_df.to_csv(
            self.predictions_summary_filename(data_version), index=True
        )
        ...
        if not resp_df is None:
            ...
            precisions_df.to_csv(
                self.prediction_precisions_filename(data_version), index=False
            )
        ...
        if not resp_df is None:
            ...
            self._model_logger.info(
                "{1} msqe: {0:.3f}".format(
                    mean_squared_error(orig_resp_df, orig_pred_df), legend
                )
            )
            self._model_logger.info(
                "{1} r2_score: {0:.3f}".format(
                    r2_score(orig_resp_df, orig_pred_df), legend
                )
            )
            if prediction_plots:
                evaluate_prediction(
                    ...
                    data_version,
                    interactive_plots,
                    out_prefix=self.report_file_prefix,
                    log_scale=False,
                )
    ...
    def model_train(
        ...
        if algo == "nn_keras":
            ...
            sample_weights_dict = self._compute_sample_weights_dict(
                y_train, sample_weights_coef, sample_weights_exp, sample_weights_int
            )
            model = self._instKeras.keras_main(
                resp_names,
                keras_algo,
                X_train,
                X_test,
            ...
            sample_weights_vect = self._compute_sample_weights_vect(
                y_train, sample_weights_coef, sample_weights_exp, sample_weights_int
            )
            model = self._instSklearn.sklearn_main(
                self.get_model_file_prefix,
                feat_names_dict,
                resp_names,
                sklearn_algo,
                X_train,
            ...
            model = self._instCaret.caret_main(
                self.get_model_file_prefix,
                feat_names_dict,
                resp_names,
                caret_algo,
    ...
    def _model_predict(
        ...
        if model_lib in ["keras", "sklearn"] and not model_per_response:
            ...
            for rn in model.keys():
                ...
                if model_lib == "caret":
                    y_pred[rn] = list(
                        caret_predict_model(model[rn], data=X)["prediction_label"]
                    )
                elif model_lib in ["keras", "sklearn"]:
                    if algo == "poly_sklearn":
                        # print('X', X.columns.tolist())
                        y_pred[rn] = self._instSklearn.poly_sklearn_predict(
                            model[rn], X
                        )
            ...
            raise Exception(
                "Unsupported model_lib "
                + str(model_lib)
                + " in function _model_predict"
            )
    ...
    def build_models(
            ...
            if model_lib == "sklearn":
                ...
                model = dict(
                    [
                        (
                            resp_name,
                            caret_load_model(self.model_filename(algo, "", resp_name)),
                        )
                ...
                if model_per_response:
                    if model_rerun_config_dict is not None:
                        ...
                        assert (
                            model_rerun_config_dict["model_per_response"]
                            == model_per_response
                        )
                    ...
                    model = dict(
                        [
                            (
                                resp_name,
                                keras_load_model(
                                    self.model_filename(algo, ".h5", resp_name)
                                ),
                            )
                            for resp_name in resp_names
                        ]
        ...
        if not X_train is None and not y_train is None:
            ...
            y_train_pred = self._model_predict(
                model, X_train, y_train, resp_names, algo, model_per_response
            )
        ...
        if not X_test is None and not y_test is None:
            ...
            # print('(3)'); print('y\n', y);  print('y_train\n', y_train); print('y_test\n', y_test);
            y_test_pred = self._model_predict(
        ...
        if X_new is not None:
            ...
            # print('(5)'); print('y_new\n', y_new)
            y_new_pred = self._model_predict(
