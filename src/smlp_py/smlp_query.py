...
class SmlpQuery:
    def __init__(self):
        ...
        self.query_params_dict = {
            "query_names": {
                "abbr": "quer_names",
                "default": str(self._DEF_QUERY_EXPRS),
                "type": str,
                "help": "Names of optimization objectives [default {}]".format(
                    str(self._DEF_QUERY_NAMES)
                ),
            },
            "query_expressions": {
                "abbr": "quer_exprs",
                "default": self._DEF_QUERY_EXPRS,
                "type": str,
            ...
            "lemma_precision": {
                "abbr": "lemma_prec",
                "default": self._DEF_LEMMA_PRECISION,
                "type": int,
    ...
    def find_candidate(self, solver):
        ...
        res = self._modelTermsInst.smlp_solver_check(
            solver, "ca", self._lemma_precision
        )
        if self._modelTermsInst.solver_status_unknown(
            res
        ):  # isinstance(res, smlp.unknown):
            return None
        else:
            return res

    def update_consistecy_results(
        self,
        mode_status_dict,
    ...
    def get_model_exploration_base_components(
        self,
        mode_status_dict,
        results_file,
        syst_expr_dict: dict,
    ...
    def check_concrete_witness_consistency(
        self,
        domain: smlp.domain,
        ...
        eta: smlp.form2,
        query: smlp.form2,
        witn_form: smlp.form2,
        solver_logic: str,
    ):
        solver = (
            self._modelTermsInst.create_model_exploration_instance_from_smlp_components(
    ...
    #   ! ( ! theta x y \/ ! alpha y \/ beta y /\ obj y >= T ) =
    #   theta x y /\ alpha y /\ ! ( beta y /\ obj y >= T)
    def find_candidate_counter_example(
        self,
        universal,
        domain: smlp.domain,
        cand: dict,
        ...
        theta = self._modelTermsInst.compute_stability_formula_theta(
            cand, None, theta_radii_dict, universal
        )
        solver.add(theta)  # print('adding theta', theta)
        solver.add(alpha)  # print('adding alpha', alpha)
        solver.add(
            self._smlpTermsInst.smlp_not(query)
        )  # print('adding negated quert', query)
        return self._modelTermsInst.smlp_solver_check(
            solver, "ce", self._lemma_precision
        )
        # return solver.check()
    ...
    def validate_witness_smt(
        ...
        eta: smlp.form2,
        alpha: smlp.form2,
        theta_radii_dict: dict,  # beta:smlp.form2,
        delta: float,
        solver_logic: str,
        witn: bool,
        sat_approx: bool,
        ...
        if universal:
            ...
            self._query_logger.info(
                "Certifying stability of witness for query "
                + str(quer_name)
                + ":\n   "
                + str(witn_dict)
            )
        ...
        for var, val in witn_dict.items():
            ...
            candidate_solver.add(
                self._smlpTermsInst.smlp_eq(smlp.Var(var), smlp.Cnst(val))
            )

        candidate_check_res = self._modelTermsInst.smlp_solver_check(
            candidate_solver, "ca"
        )
        if self._modelTermsInst.solver_status_sat(
            candidate_check_res
        ):  # isinstance(candidate_check_res, smlp.sat):
            cond_feasible = True
            if universal:
                self._query_logger.info(
                    "The configuration is consistent with assertion " + str(quer_name)
                )
            else:
                self._query_logger.info(
                    "Witness to query "
                    + str(quer_name)
                    + " is a valid witness; checking its stability"
                )
        elif self._modelTermsInst.solver_status_unsat(
            candidate_check_res
        ):  # isinstance(candidate_check_res, smlp.unsat):
            cond_feasible = False
            if universal:
                ...
                self._query_logger.info(
                    "The configuration is inconsistent with assertion " + str(quer_name)
                )
                ...
                self._query_logger.info(
                    "Witness to query "
                    + str(quer_name)
                    + " is not a valid witness (even without stability requirements)"
                )
                return "not a witness"
        else:
            raise Exception(
                "Unexpected counter-example status "
                + str(ce)
                + " in function validate_witness_smt"
            )

        # checking stability of a valid witness to the query
        witn_term_dict = self._smlpTermsInst.witness_const_to_term(witn_dict)
        ce = self.find_candidate_counter_example(
            universal,
            domain,
            witn_term_dict,
            quer,
        ...
        if self._modelTermsInst.solver_status_sat(ce):  # isinstance(ce, smlp.sat):
            if universal:
                ...
                return {
                    ...
                    "asrt": False,
                    "assertion_feasible": cond_feasible,
                    "counter_example": self._smlpTermsInst.witness_term_to_const(
                        ce_model, approximate=sat_approx, precision=sat_precision
                    ),
                }
            else:
                self._query_logger.info(
                    "Witness to query "
                    + str(quer_name)
                    + " is not stable for radii "
                    + str(theta_radii_dict)
                )
            ...
            raise Exception(
                "Unexpected counter-example ststus "
                + str(ce)
                + " in function validate_witness_smt"
            )
    ...
    def validate_witness(
        ...
        if quer_names is None or quer_exprs is None:
            ...
            self._query_logger.info(
                'Queries are not specified in mode "certify"; aborting...'
            )
            return
        ...
        (
            ...
            interface_consistent,
            model_consistent,
        ) = self.get_model_exploration_base_components(
            mode_status_dict,
            results_file,
            syst_expr_dict,
            algo,
        ...
        if witn_count != len(quer_exprs):
            raise Exception(
                "The number of queries does not match the number of witnesses"
            )
            ...
            for var, val in w.items():
                if universal:  # univeral witness, verification, synthesis and optimized synthesis modes
                    ...
                    if var in feat_names:
                        witn_dict_filtered[q][var] = val
            witn_form_dict[q] = self._smlpTermsInst.witness_to_formula(
                witn_dict_filtered[q]
            )
        ...
        if beta_expr is not None:
            quer_exprs = [
                quer_exprs[i] + " and " + beta_expr for i in range(witn_count)
            ]  # print('quer_exprs', quer_exprs)
        quer_forms_dict = dict(
            [
                (quer_name, self._smlpTermsInst.ast_expr_to_term(quer_expr))
        ...
        for i, form in enumerate(quer_forms_dict.values()):
            if not isinstance(form, smlp.libsmlp.form2):
                raise Exception(
                    "Quesry " + str(quer_exprs[i]) + " must be a formula (not a term)"
                )
        ...
        if vacuity:
            ...
            for quer_name, quer_form in quer_forms_dict.items():
                ...
                if universal:
                    ...
                    self._query_logger.info(
                        "Verifying consistency of configuration for assertion "
                        + str(quer_name)
                        + ":\n   "
                        + str(witn_form)
                    )
                    ...
                    self._query_logger.info(
                        "Certifying consistency of witness for query "
                        + str(quer_name)
                        + ":\n   "
                        + str(witn_form)
                    )
                witn_status = self.check_concrete_witness_consistency(
                    domain,
                    model_full_term_dict,
                    alpha,
                    eta,
                ...
                if self._modelTermsInst.solver_status_sat(
                    witn_status
                ):  # isinstance(witn_status, smlp.sat):
                    if universal:
                        self._query_logger.info(
                            "Input, knob and configuration constraints are consistent"
                        )
                    else:
                        self._query_logger.info(
                            "Input, knob and concrete witness constraints are consistent"
                        )
                    mode_status_dict[quer_name][CONSISTENCY] = "true"
                elif self._modelTermsInst.solver_status_unsat(
                    witn_status
                ):  # isinstance(witn_status, smlp.unsat):
                    if universal:
                        self._query_logger.info(
                            "Input, knob and configuration constraints are inconsistent"
                        )
                    else:
                        self._query_logger.info(
                            "Input, knob and concrete witness constraints are inconsistent"
                        )
                    mode_status_dict[quer_name][CONSISTENCY] = "false"

                else:
                    raise Exception(
                        "Input, knob and concrete witness cosnsistency check failed to complete"
                    )
        ...
        for quer_name, quer_form in quer_forms_dict.items():
            ...
            if mode_status_dict[quer_name][CONSISTENCY] != "false":
                ...
                witness_status_str = self.validate_witness_smt(
                    universal,
                    model_full_term_dict,
                    quer_name,
                    quer_expr_dict[quer_name],
                    quer_form,
                    witn_i_dict,
                    domain,
                    eta,
                ...
                if universal:
                    ...
                    if witness_status_str == "not a witness":
                        ...
                        raise Exception(
                            "Unexpected status "
                            + str()
                            + " in function validate_witness"
                        )
            else:
                if universal:
                    self._query_logger.info(
                        "Skipping verification of assertion {} <-> {}".format(
                            str(quer_name), str(quer_form)
                        )
                    )
                    self._query_logger.info("Reporting result: ERROR")
                    mode_status_dict[quer_name]["assertion_feasible"] = "false"
                    mode_status_dict[quer_name]["assertion_status"] = (
                        "ERROR"  # VACUOUS PASS
                    )
    ...
    def query_condition(
        ...
        if quer_expr is not None:
            self._query_logger.info(
                "Querying condition {} <-> {}".format(str(quer_name), str(quer_expr))
            )
        else:
            self._query_logger.info(
                "Querying condition {} <-> {}".format(str(quer_name), str(quer))
            )
        ...
        self._query_tracer.info(
            "{},{}".format("synthesis" if universal else "query", str(quer_name))
        )  # , str(quer_expr) ,{}
        ...
        smlp_processing_start = perf_counter()
        while True:
            ...
            if self._modelTermsInst.solver_status_sat(ca):  # isinstance(ca, smlp.sat):
                ...
                if use_approxiamted_fractions:
                    ca_model_approx = self._smlpTermsInst.approximate_witness_term(
                        ca_model, self._lemma_precision
                    )
                    # print('ca_model_approx -------------', ca_model_approx)
                    knob_vals = [
                        v for k, v in ca_model_approx.items() if k in theta_radii_dict
                    ]  # print('knob_vals', knob_vals)
                ...
                if self._modelTermsInst.solver_status_sat(
                    ...
                    for var in ce_model.keys():
                        if var in model_full_term_dict.keys():
                            del cem[var]
                    if use_approxiamted_fractions:
                        ce_model_approx = self._smlpTermsInst.approximate_witness_term(
                            cem, self._lemma_precision
                        )
                        # print('ce_model_approx ++++++++++', ce_model_approx)
                        knob_vals = [
                            v
                            for k, v in ce_model_approx.items()
                            if k in theta_radii_dict
                        ]  # print('knob_vals', knob_vals)
                        h = hash(str(knob_vals))
                        ...
                        # print('ce_model_approx', ce_model_approx)
                        lemma = self.generalize_counter_example(
                            ce_model_approx
                        )  # print('lemma', lemma)
                    else:
                        lemma = self.generalize_counter_example(
                    ...
                    theta = self._modelTermsInst.compute_stability_formula_theta(
                        lemma, delta, theta_radii_dict, universal
                    )
                ...
                self._query_logger.info(
                    "Query completed with result: UNSAT (unsatisfiable)"
                )
                if feasible is None:
                    feasible = False
                ...
                raise Exception(
                    ...
                    + str(type(ca))
                    + " in candidate search for queries"
                )
    ...
    def query_conditions(
        ...
        for i, (quer_name, quer_form) in enumerate(quer_forms_dict.items()):
            quer_res_dict[quer_name] = self.query_condition(
                universal,
                model_full_term_dict,
                quer_name,
                quer_exprs[i],
    ...
    def smlp_query(
        ...
        if quer_exprs is None:
            self._query_logger.error(
                'Queries were not specified in the "query" mode: aborting...'
            )
            mode_status_dict["smlp_execution"] = "aborted"
            with open(self.query_results_file, "w") as f:
                json.dump(mode_status_dict, f, indent="\t", cls=np_JSONEncoder)
        ...
        for i, form in enumerate(quer_forms_dict.values()):
            if not isinstance(form, smlp.libsmlp.form2):
                raise Exception(
                    "Query " + str(quer_exprs[i]) + " must be a formula (not a term)"
                )
        # execute queries
        ...
        for quer_name in quer_names:
            mode_status_dict[quer_name][QUERY_FEASIBLE] = str(
                quer_res_dict[quer_name]["feasible"]
            ).lower()
            mode_status_dict[quer_name][QUERY_STABLE] = str(
                quer_res_dict[quer_name]["query_status"] == "STABLE_SAT"
            ).lower()
            mode_status_dict[quer_name][QUERY_STATUS] = (
                "PASS"
                if quer_res_dict[quer_name]["query_status"] == "STABLE_SAT"
                else "FAIL"
            )
            mode_status_dict[quer_name][QUERY_WITNESS] = quer_res_dict[quer_name][
                "witness"
            ]
    ...
    def smlp_synthesize(
        ...
        syst_expr_dict: dict,
        algo: str,
        model: dict,  # X:pd.DataFrame, y:pd.DataFrame,
        model_features_dict: dict,
        feat_names: list[str],
        resp_names: list[str],
        asrt_names,
        ...
        data_scaler: str,
        scale_feat: bool,
        scale_resp: bool,  # scale_objv:bool,
        float_approx=True,
        float_precision=64,
        data_bounds_json_path=None,
        bounds_factor=None,
        ...
        if asrt_exprs is not None:
            ...
            synth_cond_list = (
                asrt_exprs if beta_expr is None else [beta_expr] + asrt_exprs
            )
        ...
        mode_status_dict[QUERY_STABLE] = str(
            quer_res["query_status"] == "STABLE_SAT"
        ).lower()
        mode_status_dict[QUERY_STATUS] = (
            "PASS" if quer_res["query_status"] == "STABLE_SAT" else "FAIL"
        )

        if quer_res["query_status"] == "UNSAT":  #  synthesis FAIL / not feasible
            self._query_logger.info(
                "Model configuration synthesis is infeasible under given constraints"
            )
        elif quer_res["query_status"] == "STABLE_SAT":  #  synthesis PASS / successful
            witness_vals_dict = quer_res["witness"]
            self._query_logger.info(
                "Model configuration synthesis completed successfully"
            )
