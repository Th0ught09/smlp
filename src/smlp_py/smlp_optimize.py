...
class SmlpOptimize:
    def __init__(self):
        ...
        self.opt_params_dict = {
            ...
            "center_offset": {
                "abbr": "center_offset",
                "default": self._DEF_CENTER_OFFSET,
                "type": str,
            ...
            "objectives_names": {
                "abbr": "objv_names",
                "default": str(self._DEF_OBJECTIVES_NAMES),
                "type": str,
                "help": "Names of optimization objectives [default {}]".format(
                    str(self._DEF_OBJECTIVES_NAMES)
                ),
            },
            "objectives_expressions": {
                "abbr": "objv_exprs",
                "default": self._DEF_OBJECTIVES_EXPRS,
                "type": str,
            ...
            "scale_objectives": {
                "abbr": "scale_objv",
                "default": self._DEF_SCALE_OBJECTIVES,
                "type": str_to_bool,
                "help": "Should optimization objectives be scaled using scaler specified through "
                + 'option "data_scaler"? [default: '
                + str(self._DEF_SCALE_OBJECTIVES)
                + "]",
            },
            "optimize_pareto": {
                "abbr": "pareto",
                "default": self._OPTIMIZE_PARETO,
                "type": str_to_bool,
                "help": "Should optimization be per objective (even if there are multiple objectives) "
                + "or pareto optimization must be performed? "
                + "[default: "
                + str(self._OPTIMIZE_PARETO)
                + "]",
            },
            "approximate_fractions": {
                "abbr": "frac_aprox",
                "default": self._DEF_APPROXIMATE_FRACTIONS,
                "type": str_to_bool,
                "help": "Should fraction values form satisfying assignments be converted to approximate reals? "
                + "[default: "
                + str(self._DEF_APPROXIMATE_FRACTIONS)
                + "]",
            },
            "fraction_precision": {
                "abbr": "frac_prec",
                "default": str(self._DEF_FRACTION_PRECISION),
                "type": int,
                "help": "Decimal precision when approximating fractions by reals [default {}]".format(
                    str(self._DEF_FRACTION_PRECISION)
                ),
            },
            "vacuity_check": {
                "abbr": "vacuity",
                "default": self._DEF_VACUITY_CHECK,
                "type": str_to_bool,
                ...
                + '"optimize" and "optsyn" modes [default: '
                + str(self._DEF_VACUITY_CHECK)
                + "]",
            },
            "optimization_strategy": {
                "abbr": "opt_strategy",
                "default": str(self._DEF_OPTIMIZATION_STRATEGY),
                "type": str,
                "help": 'Strategy (algorithm) to use for single objective optimization in the "optimize" and "optsyn" modes. '
                + 'Supported options are "lazy" and "eager" [default {}]'.format(
                    str(self._DEF_OPTIMIZATION_STRATEGY)
                ),
            },
        }
    ...
    def compute_objectives_bounds(
        ...
        if X is None or y is None:
            self._opt_logger.warning(
                "No training data available (using saved model). "
                "Computing objective bounds using wide range estimates."
            )
            objv_bounds_dict = {}
            # For each objective, use a wide range: [-1e6, 1e6]
            # This allows the optimization to proceed but may not be tight bounds
            for objv_name in objv_names:
                objv_bounds_dict[objv_name] = {
                    "min": -1e6,
                    "max": 1e6,
                }
        ...
        def eval_objv(row):
            ...
            res_row = eval(
                objv_cond, {}, eval_env
            )  # print('res_row', res_row, type(res_row))
            return res_row
        ...
        for i, (objv_name, objv_cond) in enumerate(zip(objv_names, objv_exprs)):
            ...
            objv_series = df_resp_feat.apply(
                lambda row: eval_objv(row), axis=1
            )  # print('objv_series', objv_series.tolist())
            objv_bounds_dict[objv_name] = {
                "min": float(objv_series.min()),
                "max": float(objv_series.max()),
            }
    ...
    def bin_opt_max(
        self,
        smlp_domain: smlp.domain,
        model_full_term_dict: dict,
        phi_cond: smlp.form2,
        objv_name: str,
        objv_expr: str,
        objv_term: smlp.term2,
        l0: float,
        u0: float,
        epsilon: float,
        solver_logic: str,
    ):
        self._opt_tracer.info(
            "bin_opt_max start, objective_thresholds_u0_l0, {} : {}".format(
                str(u0), str(l0)
            )
        )
        assert l0 < u0
        ...
        while True:
            self._opt_tracer.info(
                "bin_opt_max iter, objective_thresholds_u0_l0_u_l, {} : {} : {} : {}".format(
                    str(u0), str(l0), str(u), str(l)
                )
            )
            ...
            quer_form = self._smlpTermsInst.smlp_ge(
                objv_term, smlp.Cnst(T)
            )  # print('quer_form', quer_form)
            quer_expr = (
                "{} >= {}".format(objv_expr, str(T)) if objv_expr is not None else None
            )
            ...
            self._opt_tracer.info(
                "adjusted thresholds, objective_thresholds_u0_l0_u_l_T, {} : {} : {} : {} : {}".format(
                    str(u0), str(l0), str(u), str(l), str(T)
                )
            )
            ...
            res = self._queryInst._modelTermsInst.smlp_solver_check(
                max_solver, "max_ca", self._queryInst._lemma_precision
            ...
            if self._modelTermsInst.solver_status_unsat(res):
                ...
                l = T
                p_cand = self._modelTermsInst.get_solver_knobs_model(
                    res
                )  # print('p_cand', p_cand, flush=True)
        ...
        if self._trace_precision > 0:
            p_cand_trace = self._modelTermsInst.witness_term_to_const(
                p_cand, approximate=True, precision=self._trace_precision
            )
        else:
            p_cand_trace = self._modelTermsInst.witness_term_to_const(
                p_cand, approximate=False, precision=None
            )
        self._opt_tracer.info(
            "bin_opt_max end, p_cand_l_u, {} : {} : {}".format(
                str(p_cand_trace), str(l), str(u)
            )
        )
    ...
    def bin_opt_min(
        ...
        self._opt_tracer.info(
            "bin_opt_min start, objective_thresholds_u0_l0, {} : {}".format(
                str(u0), str(l0)
            )
        )
        assert l0 < u0
        assert l0 not in [-np.inf, np.inf]
        assert u0 not in [-np.inf, np.inf]
        p_cand, y_cand, l_prime, u_prime = self.bin_opt_max(
            smlp_domain,
            model_full_term_dict,
            phi_cond,
            "_negated_" + objv_name,
            None if objv_expr is None else "-(" + objv_expr + ")",
            self._smlpTermsInst.smlp_neg(objv_term),
            (-u0),
            (-l0),
            epsilon,
            solver_logic,
        ...
        self._opt_tracer.info(
            "bin_opt_min end, p_cand_l_u, {} : {} : {}".format(
                str(p_cand_trace), str((-u_prime)), str((-l_prime))
            )
        )
    ...
    def optimize_single_objective_eager(
        self,
        model_full_term_dict: dict,
        objv_name: str,
        objv_expr: str,
        ...
        smlp_domain: smlp.domain,
        eta: smlp.form2,
        theta_radii_dict: dict,
        alpha: smlp.form2,
        beta: smlp.form2,
        delta: float,
        solver_logic: str,
        scale_objectives: bool,
        orig_objv_name: str,
        objv_bounds: dict,
        call_info=None,
        sat_approx=False,
        ...
        l0=None,
        u0=None,
    ):  # , l=(-np.inf), u=np.inf): #TODO !!! l, u are not needed???
        self._opt_logger.info(
            "Optimize single objective with eager strategy "
            + str(objv_name)
            + ": Start"
        )
        ...
        self._opt_tracer.info(
            "single_objective_u0_l0, {} : {}".format(str(u0), str(l0))
        )
        # print('eta', eta); print('alpha', alpha); print('beta', beta); print('objv_term', objv_term)
        phi_cand = self._smlpTermsInst.smlp_and_multi(
            [eta, alpha, beta]
        )  # print('phi_cand', phi_cand)
        ...
        u_cand = (
            np.inf
        )  # upper bound for candidate # TODO !!! use argument u or delete this argument
        l_stab = (
            -np.inf
        )  # lower bound for stability # TODO !!! use argument l or delete this argument
        ...
        while True:
            self._opt_tracer.info(
                "eager opt iter, objective_thresholds_ue_le_ucand_lstab, {} : {} : {} : {}".format(
                    str(u_e), str(l_e), str(u_cand), str(l_stab)
                )
            )
            if l_stab + epsilon > u_cand:
                self._opt_tracer.info(
                    "eager opt end, optimization converged with required accuracy"
                )
                break
            phi_cond = self._smlpTermsInst.smlp_and(
                phi_cand, L
            )  # print('phi_cond', phi_oand)
            if self._tmp_use_trace_lemmas:
                self._opt_tracer.info(
                    "eager opt iter, phi_cond lemma, {}".format(str(phi_cond))
                )
            ...
            res = self._queryInst._modelTermsInst.smlp_solver_check(
                candidate_solver, "ca_eager", self._queryInst._lemma_precision
            )
            assert res is not None
            assert not self._modelTermsInst.solver_status_unknown(res)
            if self._modelTermsInst.solver_status_unsat(res):
                self._opt_tracer.info(
                    "eager opt end, optimization result cannot be improved"
                )
                break
            p_cand, y_cand, l_cand, u_cand = self.bin_opt_max(
                smlp_domain,
                model_full_term_dict,
                phi_cond,
                objv_name,
            ...
            theta_p_cand = self._modelTermsInst.compute_stability_formula_theta(
                p_cand, None, theta_radii_dict, True
            )
            ...
            if l_stab == (-np.inf):
                ...
                low_thresh_form = self._smlpTermsInst.smlp_le(
                    objv_term, self._smlpTermsInst.smlp_cnst(l_stab)
                )
            # print('low_thresh_form', low_thresh_form, flush=True)
            beta_implies_thresh_form = self._smlpTermsInst.smlp_implies(
                beta, low_thresh_form
            )
            ...
            res = self._modelTermsInst.smlp_solver_check(
                cex_cand_solver, "phi_cex_cand", self._queryInst._lemma_precision
            )
            assert not self._modelTermsInst.solver_status_unknown(res)
            if self._modelTermsInst.solver_status_sat(res):
                p_cex, y_cex = (
                    self._modelTermsInst.get_solver_knobs_model(res),
                    self._modelTermsInst.get_solver_resps_model(res),
                )
                ...
                p_cex, y_cex, l_stab, u_cex = self.bin_opt_min(
                    smlp_domain,
                    model_full_term_dict,
                    phi_cex,
                    objv_name,
                ...
                self._opt_logger.info(
                    "Increasing threshold lower bound for objective "
                    + str(objv_name)
                    + " from "
                    + str(l_stab_prev)
                    + " to "
                ...
                objv_witn_val_term = smlp.subst(
                    objv_term, stable_witness_terms
                )  # print('objv_witn_val_term', objv_witn_val_term)
                update_progress_report = l_stab_prev != -np.inf
                stable_witness_vals = self.update_optimization_reports(
                    stable_witness_terms,
                    l_stab,
                    u_e,
                    call_info,
            ...
            theta_p_cex_delta = self._modelTermsInst.compute_stability_formula_theta(
                p_cex, delta, theta_radii_dict, True
            )
            if self._tmp_use_trace_lemmas:
                self._opt_tracer.info(
                    "eager opt iter, edding lemma, {}".format(str(theta_p_cex_delta))
                )
            L = self._smlpTermsInst.smlp_and(
                L, self._smlpTermsInst.smlp_not(theta_p_cex_delta)
            )
            iter_count += +1
        ...
        # print('P', P)
        l_res = max(
            [tup[3] for tup in P_eager]
        )  # print('l_res', l_res, 'threshold_lo', P[-1]['threshold_lo'])
        assert l_res == P[-1]["threshold_lo_scaled"]
        # print('optimize_single_objective_eager and with l_res', l_res, flush=True)
        self._opt_logger.info(
            "Optimize single objective with eager strategy " + str(objv_name) + ": end"
        )
        return P[-1]  # l_res
    ...
    def optimize_single_objective(
        ...
        self._opt_tracer.info(
            "single_objective_u0_l0_u_l, {} : {} : {} : {} : {}".format(
                str(objv_name), str(u0), str(l0), str(u), str(l)
            )
        )
        ...
        while True:
            ...
            quer_name = objv_name + "_" + str(T)
            quer_and_beta = (
                self._smlpTermsInst.smlp_and(quer_form, beta)
                if not beta == smlp.true
                else quer_form
            )
            # print('quer_and_beta', quer_and_beta) 'u0_l0_u_l_T'
            self._opt_tracer.info(
                "objective_thresholds_u0_l0_u_l_T, {} : {} : {} : {} : {}".format(
                    str(u0), str(l0), str(u), str(l), str(T)
                )
            )
            ...
            if stable_witness_status == "UNSAT":
                assert T <= u
                self._opt_logger.info(
                    "Decreasing threshold upper bound for objective "
                    + str(objv_name)
                    + " from "
                    + str(u)
                    + " to "
                ...
                if len(P) == 0:
                    ...
                    if l not in [np.inf, -np.inf]:
                        unscaled_threshold_lo = (
                            self._scalerTermsInst.unscale_constant_term(
                                objv_bounds, orig_objv_name, l
                            )
                        )
                    ...
                    if u not in [np.inf, -np.inf]:
                        unscaled_threshold_up = (
                            self._scalerTermsInst.unscale_constant_term(
                                objv_bounds, orig_objv_name, u
                            )
                        )
                ...
                raise Exception(
                    "Unsupported value "
                    + str(stable_witness_status)
                    + " received from query_conditions"
                )
        ...
        if u not in [np.inf, -np.inf]:
            ...
            if scale_objectives:
                ...
                P[-1]["threshold_up"] = orig_min + u * (
                    orig_max - orig_min
                )  # print( P[-1]['threshold_up'])
            else:
    ...
    def optimize_single_objectives(
        self,
        feat_names: list,
        resp_names: list,  # X:pd.DataFrame, y:pd.DataFrame,
        model_full_term_dict: dict,
        objv_names: list,
        objv_exprs: list,
        objv_bounds_dict: dict,
        alpha: smlp.form2,
        beta: smlp.form2,
        eta: smlp.form2,
        theta_radii_dict: dict,
        ...
        objv_terms_dict, orig_objv_terms_dict, scaled_objv_terms_dict = (
            self._modelTermsInst.compute_objectives_terms(
                objv_names, objv_exprs, objv_bounds_dict, scale_objectives
            )
        )
        ...
        for i, (objv_name, objv_term) in enumerate(list(objv_terms_dict.items())):
            ...
            if scale_objectives:
                ...
                objv_epsn = self.unscale_relative_constant_val(
                    objv_bounds_dict, objv_names[i], epsilon
                )
            # print('objv_epsn', objv_epsn)
            if strategy == "lazy":
                opt_conf[objv_names[i]] = self.optimize_single_objective(
                    model_full_term_dict,
                    objv_name,
                    objv_expr,
                    objv_term,
                    ...
                    sat_precision=64,
                    save_trace=False,
                )  # print('opt_conf', opt_conf)
            elif strategy == "eager":
                opt_conf[objv_names[i]] = self.optimize_single_objective_eager(
                    model_full_term_dict,
                    objv_name,
                    objv_expr,
                    objv_term,
        ...
        with open(self.optimization_results_file + ".json", "w") as f:
            json.dump(
                opt_conf | self.mode_status_dict, f, indent="\t", cls=np_JSONEncoder
            )
    ...
    def active_objectives_max_min_bounds(
        self,
        model_full_term_dict: dict,
        objv_terms_dict: dict,
        t: list[float],
        smlp_domain: smlp.domain,
        alpha: smlp.form2,
        beta: smlp.form2,
        ...
        for j, (objv_name, objv_term) in enumerate(objv_terms_dict.items()):
            if t[j] is not None:
                eta_F_t = self._smlpTermsInst.smlp_and(
                    eta_F_t, objv_term > smlp.Cnst(t[j])
                )
            else:
        ...
        if strategy == "lazy":
            r = self.optimize_single_objective(
                model_full_term_dict,
                min_name,
                None,
                min_objs,
            ...
            r = self.optimize_single_objective_eager(
                model_full_term_dict,
                min_name,
                None,
                min_objs,
        ...
        c_up = (
            r["threshold_up"] if "threshold_up" in r else np.inf
        )  # print('c_up', c_up)
    ...
    def prog_dict_to_df(self):
        ...
        for k, v in prog_dict.items():
            ...
            feat_df = v_new_df[
                sorted([col for col in v_new_df.columns if col in self.feat_names])
            ]
            resp_df = v_new_df[
                sorted([col for col in v_new_df.columns if col in self.resp_names])
            ]
            objv_df = v_new_df[
                sorted([col for col in v_new_df.columns if col in self.objv_names])
            ]
    ...
    def report_current_thresholds(
        ...
        if scale_objv:
            s_origin = [
                self.unscale_constant_val(objv_bounds_dict, objv_name, b)
                for (objv_name, b) in zip(objv_names, s)
            ]  # print('s_unscaled', s_unscaled)
            s_scaled = s
        else:
            s_origin = s
            s_scaled = [
                self.scale_constant_val(objv_bounds_dict, objv_name, b)
                for (objv_name, b) in zip(objv_names, s)
            ]  # print('s_unscaled', s_unscaled)
        s_scaled_str = [
            "%.6f" % e for e in s_scaled
        ]  # print('s_scaled_str', s_scaled_str)
        s_origin_str = [
            "%.6f" % e for e in s_origin
        ]  # print('s_origin_str', s_origin_str)
        s_scaled_dict = dict(zip(objv_names, s_scaled_str))
        s_origin_dict = dict(zip(objv_names, s_origin_str))
        if completed:
            key_label = "final"
            self._opt_logger.info(
                "Pareto optimization completed with objectives thresholds: "
                + "\n    Scaled to [0,1]: {}\n    Original  scale: {}\n".format(
                    s_scaled_dict, s_origin_dict
                )
            )
        elif call_n is None:
            key_label = "vacuity"
            self._opt_logger.info(
                "Pareto optimization vacuity completed with objectives thresholds: "
                + "\n    Scaled to [0,1]: {}\n    Original  scale: {}\n".format(
                    s_scaled_dict, s_origin_dict
                )
            )
            ...
            self._opt_logger.info(
                "Pareto optimization in progress with objectives thresholds: "
                + "\n    Scaled to [0,1]: {}\n    Original  scale: {}\n".format(
                    s_scaled_dict, s_origin_dict
                )
            )
        ...
        if witness_vals_dict is not None:
            objv_vals_dict = dict(
                [
                    (objv_name, eval(objv_expr, {}, witness_vals_dict))
            ...
            for i, objv_name in enumerate(objv_names):
                ...
                self.best_config_dict[key_label][objv_names[i]] = {
                    ...
                    "threshold_scaled": s_scaled[i],
                    "threshold": s_origin[i],
            ...
            for key, val in witness_vals_dict.items():
                ...
                if key in self.resp_names and self.syst_expr_dict is not None:
                    if key in self.syst_expr_dict.keys():
                        ...
                        system_val = eval(
                            self.syst_expr_dict[key], {}, witness_vals_dict
                        )  # print('system_val', system_val)
                        self.best_config_dict[key_label][key]["value_in_system"] = (
                            system_val
                        )
        ...
        with open(
            self.optimization_progress_file + ".json", "w"
        ) as f:  # json.dump(asrt_res_dict, f)
            json.dump(self.best_config_dict, f, indent="\t", cls=np_JSONEncoder)
        self.best_config_df.to_csv(
            self.optimization_progress_file + ".csv", index=False
        )
        if completed:
            ...
            with open(self.optimization_results_file + ".json", "w") as f:
                json.dump(
                    self.best_config_dict["final"] | self.mode_status_dict,
                    f,
                    indent="\t",
                    cls=np_JSONEncoder,
                )
            final_config_df = self.best_config_df.drop_duplicates(
                subset=self.feat_names, inplace=False, keep="last"
            )
            final_config_df.to_csv(self.optimization_results_file + ".csv", index=False)

    def update_optimization_reports(
        self,
        stable_witness_terms,
        l,
        u,
        ...
        objv_expr,
        orig_objv_name,
        update_progress_report: bool,
        sat_approx,
        sat_precision,
    ):
        stable_witness_terms[objv_name] = objv_witn_val_term
        if (
            call_info is not None
            and call_info["update_thresholds"]
            and update_progress_report
        ):
            witness_vals_dict = self._smlpTermsInst.witness_term_to_const(
                stable_witness_terms, sat_approx, sat_precision
            )  # print('witness_vals_dict', witness_vals_dict)
            # if objv_name in witness_vals_dict:
            del witness_vals_dict[
                objv_name
            ]  # print('witness_vals_dict after del', witness_vals_dict)
            ...
            self.report_current_thresholds(
                s,
                witness_vals_dict,
                self.objv_bounds_dict,
                self.objv_names,
                self.objv_exprs,
                False,
                (call_info["global_iter"], iter_count),
                scale_objectives,
            )
        ...
        if scale_objectives:
            # print('objv_bounds', objv_bounds)
            objectives_unscaler_terms_dict = (
                self._scalerTermsInst.feature_unscaler_terms(
                    objv_bounds, [orig_objv_name]
                )
            )
            # substitute scaled objective variables with scaled objective terms
            # in original objective terms within objectives_unscaler_terms_dict
            if objv_expr is not None:
                orig_objv_const_term = smlp.subst(
                    objectives_unscaler_terms_dict[orig_objv_name],  # objv_term,
                    {
                        self._scalerTermsInst._scaled_name(
                            orig_objv_name
                        ): objv_witn_val_term
                    },
                )
                ...
                if objv_name_unscaled in self.objv_names:
                    stable_witness_terms[objv_name_unscaled] = orig_objv_const_term
            if l not in [np.inf, -np.inf]:
                unscaled_threshold_lo = self._scalerTermsInst.unscale_constant_term(
                    objv_bounds, orig_objv_name, l
                )
                stable_witness_terms["threshold_lo_scaled"] = smlp.Cnst(l)
                stable_witness_terms["threshold_lo"] = unscaled_threshold_lo
            if u not in [np.inf, -np.inf]:
                unscaled_threshold_up = self._scalerTermsInst.unscale_constant_term(
                    objv_bounds, orig_objv_name, u
                )
                # print('unscaled_threshold_up: l', l, 'unsc', unscaled_threshold_up)
                stable_witness_terms["threshold_up_scaled"] = smlp.Cnst(u)
        ...
        stable_witness_terms["max_in_data"] = smlp.Cnst(
            objv_bounds[orig_objv_name]["max"]
        )
        stable_witness_terms["min_in_data"] = smlp.Cnst(
            objv_bounds[orig_objv_name]["min"]
        )
    ...
    def optimize_pareto_objectives(
        self,
        feat_names: list[str],
        resp_names: list[str],
        model_full_term_dict: dict,
        ...
        if not scale_objectives:
            raise Exception(
                "Pareto optimization requires scale_objectives to be True and data_scaler not none"
            )
        # assert scale_objectives
        objv_terms_dict, orig_objv_terms_dict, scaled_objv_terms_dict = (
            self._modelTermsInst.compute_objectives_terms(
        ...
        while len(K) > 0:
            ...
            self._opt_tracer.info(
                "pareto_iteration,{},{},{}".format(
                    str(call_n), "__".join(objv_names), "__".join([str(e) for e in s])
                )
            )
            c_lo, c_up, witness = self.active_objectives_max_min_bounds(
                model_full_term_dict,
                objv_terms_dict,
                s,
                smlp_domain,
            ...
            t_for_loop = (
                t.copy()
            )  # using copy() here to insure changes in t do not affect t_for_loop
            ...
            for j in K:
                ...
                if raised_count == len(K) - 1:
                    ...
                    self._opt_logger.info(
                        "Checking whether to fix objective {} at threshold {}...\n".format(
                            str(j), str(s[j])
                        )
                    )
                    self._opt_tracer.info(
                        "activity check, objective {} threshold {}".format(
                            str(objv_names[j]), str(s[j])
                        )
                    )
                    ...
                    for i in objv_enum:
                        ...
                        quer_form = self._smlpTermsInst.smlp_and(
                            quer_form,
                            list(objv_terms_dict.values())[i] > smlp.Cnst(t[i]),
                        )
                    ...
                    opt_quer_name = (
                        "thresholds_" + "_".join(str(x) for x in t) + "_check"
                    )
                    quer_res = self._queryInst.query_condition(
                        True,
                        model_full_term_dict,
                ...
                if quer_res["query_status"] != "STABLE_SAT":
                    ...
                    self._opt_logger.info(
                        "Not fixing objective {} at threshold {}...\n".format(
                            str(j), str(s[j])
                        )
                    )
                    self._opt_logger.info(
                        "Lower bounds of objectives {} can be raised to threshold {}...\n".format(
                            str([objv_names[i] for i in K_pr]), str(c_lo + epsilon)
                        )
                    )
    ...
    def check_synthesis_feasibility(
        ...
        if feasibility:
            self._opt_logger.info(
                "Pareto optimization synthesis feasibility check: Start"
            )
            ...
            if quer_res["query_status"] == "UNSAT":
                self._opt_logger.info(
                    "Pareto optimization synthesis feasibility check: End"
                )
                return True, None
            elif quer_res["query_status"] == "STABLE_SAT":
                witness_vals_dict = quer_res[
                    "witness"
                ]  # print('witness_vals_dict', witness_vals_dict)
                s = []
                for objv_name, objv_expr in zip(objv_names, objv_exprs):
            ...
            self._opt_logger.info(
                "Skipping pareto optimization synthesis feasibility check"
            )
            return False, None
    ...
    def smlp_optimize(
        ...
        objv_exprs,
        pareto: bool,
        strategy: str,  # asrt_names:list[str], asrt_exprs,
        quer_names: list[str],
        quer_exprs,
        delta: float,
        epsilon: float,
        ...
        (
            ...
            data_scaler,
            scale_feat,
            scale_resp,  # scale_objv,
            float_approx,
            float_precision,
            data_bounds_json_path,
        )

        self.mode_status_dict = self._queryInst.update_consistecy_results(
            self.mode_status_dict,
            interface_consistent,
            model_consistent,
            "optimization_status",
            self.optimization_results_file + ".json",
        )
        if not interface_consistent:
            self._opt_logger.info(
                "Input and knob interface constraints are inconsistent; aborting..."
            )
            return
        elif not model_consistent:
            self._opt_logger.info(
                "Input and knob interface constraints are inconsistent with model constraints; aborting..."
            )
            return
        ...
        objv_bounds_dict = self.compute_objectives_bounds(
            X, y, objv_names, objv_exprs
        )  # print('objv_bounds_dict', objv_bounds_dict)
        self.objv_bounds_dict = objv_bounds_dict

        # instance consistency check (are the assumptions contradictory?)
        contradiction, thresholds = self.check_synthesis_feasibility(
            vacuity,
            objv_names,
            objv_exprs,
            objv_bounds_dict,
        ...
        if contradiction:
            ...
            self._opt_logger.info(
                "Model configuration optimization instance is inconsistent with synthesis constraints; aborting..."
            )
        ...
        input_features = [
            ...
            if ft in self._modelTermsInst._specInst.get_spec_inputs
        ]
        if strategy == "eager" and len(input_features) > 0:
            strategy = "lazy"

        if pareto:
            self.optimize_pareto_objectives(
                feat_names,
                resp_names,
                model_full_term_dict,  # X, y,
                objv_names,
                objv_exprs,
                objv_bounds_dict,
                alpha,
            ...
            self.optimize_single_objectives(
                feat_names,
                resp_names,
                model_full_term_dict,  # X, y,
                objv_names,
    ...
    def smlp_optsyn(
        ...
        if asrt_exprs is not None:
            ...
            asrt_forms_dict = dict(
                [
                    (asrt_name, self._smlpTermsInst.ast_expr_to_term(asrt_expr))
                    for asrt_name, asrt_expr in zip(asrt_names, asrt_exprs)
                ]
            )
            asrt_conj = self._smlpTermsInst.smlp_and_multi(
                list(asrt_forms_dict.values())
            )
        else:
            asrt_conj = smlp.true
        beta = (
            self._smlpTermsInst.smlp_and(beta, asrt_conj)
            if beta != smlp.true
            else asrt_conj
        )
        ...
        if contradiction:
            # instance is contradictory -- more precisely, stable witness does not exist; abort
            self._opt_logger.info(
                "Model configuration optimized synthesis instance is inconsistent with synthesis constraints; aborting..."
            )
            self.mode_status_dict["smlp_execution"] = "aborted"
            with open(self.optimization_results_file + ".json", "w") as f:
                json.dump(self.mode_status_dict, f, indent="\t", cls=np_JSONEncoder)
        ...
        if strategy == "eager" and len(input_features) > 0:
            self._opt_logger.warning(
                "Eager strategy on single objectives cannot be run because spec contains free input(s); the lazy strategy will be run instead..."
            )
            strategy = "lazy"
