...
class SmlpFlows:
    def __init__(self, argv):
        ...
        self.logger = self.loggerInst.create_logger(
            "smlp_logger",
            self.log_file,
            self.args.log_level,
            self.args.log_mode,
        ...
        self.supervised_modes = (
            ["subgroups", "discretize", "correlate"]
            + self.model_prediction_modes
            + self.model_exploration_modes
            + self.data_exploration_modes
        )
        ...
        if self.args.analytics_mode in self.model_exploration_modes:
            self.tracer = self.loggerInst.create_logger(
                "smlp_tracer",
                self.trace_file,
                self.args.log_level,
                self.args.log_mode,
            ...
            self.optInst.set_tracer(
                self.tracer,
                self.args.trace_runtime,
                self.args.trace_precision,
                self.args.trace_anonymize,
            )
    ...
    def data_fname(self):
        if self.args.labeled_data is None:
            ...
            not self.args.labeled_data.endswith("bz2")
            and not self.args.labeled_data.endswith("gz")
            and not self.args.labeled_data.endswith(".csv")
        ):
    ...
    def new_data_fname(self):
        if self.args.new_data is None:
            ...
            not self.args.new_data.endswith("bz2")
            and not self.args.new_data.endswith("gz")
            and not self.args.new_data.endswith(".csv")
        ):
    ...
    def smlp_flow(self):
        ...
        if args.analytics_mode in self.supervised_modes:
            if args.response is None:
                if (
                    args.analytics_mode in self.model_exploration_modes
                    or args.analytics_mode in self.data_exploration_modes
                    or (
                        args.analytics_mode in self.model_prediction_modes
                ...
                if args.analytics_mode in self.model_exploration_modes:
                    ...
                    resp_wo_spec = [
                        ...
                        if resp not in self.specInst.get_spec_responses
                    ]
                    if len(resp_wo_spec) > 0:
                        raise Exception(
                            "Response(s) "
                            + " ".join(resp_wo_spec)
                            + " are not defined in spc file"
                        )
        ...
        if (
            ...
            or (
                args.model == "system"
                and args.analytics_mode in self.model_prediction_modes
            )
        ):
            ...
            (
                ...
                args.delta_absolute,
                args.delta_relative,
                args.assertions_names,
                args.assertions_expressions,
                args.query_names,
                args.query_expressions,
                args.objectives_names,
                args.objectives_expressions,
                resp_names,
            ...
            if args.analytics_mode in ["optimize", "optsyn"]:
                ...
                for resp_name in resp_names:
                    if resp_name in objv_names:
                        raise Exception(
                            "Response (output) names must be different from names of objectives"
                        )
            self.dataInst.set_spec_inst(self.specInst)
        ...
        if args.analytics_mode == "doe" or generate_doe_data:
            doe_out_df = self.doeInst.sample_doepy(
                args.doe_algo,
                args.doe_spec_file,
                args.doe_num_samples,
                self.configInst.report_file_prefix,
                args.doe_prob_distribution,
                args.doe_design_resolution,
                args.doe_central_composite_center,
                args.doe_central_composite_face,
                args.doe_central_composite_alpha,
                args.doe_box_behnken_centers,
            )  # print('doe_out_df\n', doe_out_df);
            if args.analytics_mode == "doe":
                self.logger.info(
                    'Running SMLP in mode "{}": End'.format(args.analytics_mode)
                )
                ...
                for r, expr in syst_expr_dict.items():
                    doe_out_df[r] = doe_out_df.apply(
                        lambda row: eval(expr, {}, row), axis=1
                    )
        ...
        if args.analytics_mode == "discretize":
            X, y, feat_names, resp_names, feat_names_dict = (
                self.dataInst.preprocess_data(
                    self.data_fname,
                    feat_names,
                    resp_names,
                    None,
                    args.keep_features,
                    args.impute_responses,
                    "training",
                    args.positive_value,
                    args.negative_value,
                    args.response_map,
            ...
            self.discrInst.smlp_discretize_df(
                X,
                algo=args.discretization_algo,
                bins=args.discretization_bins,
                labels=args.discretization_labels,
        ...
        if args.analytics_mode == "correlate":
            ...
            self.correlInst.smlp_correlate(
                X,
                y,
                feat_names,
                resp_names,
                feat_names_dict,
                args.discretization_algo,
                args.discretization_bins,
                args.discretization_labels,
                args.discretization_type,
                args.discretize_numeric_features,
                args.continuous_correlation_estimators,
                args.mutual_information_method,
                args.correlations_and_mutual_information,
        ...
        if args.analytics_mode == "subgroups":
            ...
            fs_ranking_df, fs_summary_df, results_dict = self.psgInst.smlp_subgroups(
                X,
                y,
                resp_names,
                args.positive_value,
                args.negative_value,
                args.psg_quality_target,
                args.psg_max_dimension,
                args.psg_top_ranked,
        ...
        if (
            args.analytics_mode
            in self.model_prediction_modes + self.model_exploration_modes
        ):
            # self.logger.info('Running SMLP in mode "{}": Start'.format(args.analytics_mode))
            ...
            (
                ...
                args.train_first_n,
                args.train_random_n,
                args.train_uniform_n,
                args.interactive_plots,
                args.response_plots,
                args.data_scaler,
                args.scale_features,
                args.scale_responses,
                args.impute_responses,
                args.mrmr_feat_count_for_prediction,
                args.positive_value,
                args.negative_value,
                args.response_map,
                args.response_to_bool,
            ...
            if args.model == "system":
                ...
                model = self.modelInst.build_models(
                    ...
                    levels_dict,
                    model_features_dict,
                    self.modelInst.get_hyperparams_dict(args, args.model),
                    args.interactive_plots,
                    args.prediction_plots,
                    args.seed,
                    args.sample_weights_coef,
                    args.sample_weights_exponent,
                    args.sample_weights_intercept,
                    args.save_model,
                    args.use_model,
            ...
            if args.analytics_mode == "verify":
                if True or len(self.specInst.get_spec_knobs) > 0:
                    if config_dict is None:
                        ...
                        config_dict = dict(
                            [(asrt_name, configuration) for asrt_name in asrt_names]
                    ...
                    self.queryInst.smlp_verify(
                        ...
                        args.scale_features,
                        args.scale_responses,
                        args.approximate_fractions,
                        args.fraction_precision,
                        self.dataInst.data_bounds_file,
                        bounds_factor=None,
                        T_resp_bounds_csv_path=None,
                    )
                else:
                    self.verifyInst.smlp_verify(
                        syst_expr_dict,
                        args.model,
                        model,
                ...
                self.queryInst.smlp_certify(
                    syst_expr_dict,
                    args.model,
                    model,  # False, #universal
                    # self.dataInst.unscaled_training_features, self.dataInst.unscaled_training_responses,
                    model_features_dict,
                    feat_names,
                    resp_names,
                    ...
                    args.data_scaler,
                    args.scale_features,
                    args.scale_responses,  # args.scale_objectives,
                    args.approximate_fractions,
                    args.fraction_precision,
                    self.dataInst.data_bounds_file,
                    bounds_factor=None,
                ...
                self.queryInst.smlp_synthesize(
                    syst_expr_dict,
                    args.model,
                    model,
                    # self.dataInst.unscaled_training_features, self.dataInst.unscaled_training_responses,
                ...
                self.optInst.smlp_optimize(
                    syst_expr_dict,
                    args.model,
                    model,
                    self.dataInst.unscaled_training_features,
                    self.dataInst.unscaled_training_responses,
                    model_features_dict,
                    feat_names,
                    resp_names,
                    objv_names,
                ...
                if syst_expr_dict is not None:
                    if "final" in self.optInst.best_config_dict:
                        stability_region_dict = (
                            self.specInst.get_spec_stability_ragion_bounds_dict(
                                self.optInst.best_config_dict["final"]
                            )
                        )
                        self.refineInst.compute_rmse(
                            stability_region_dict,
                            model,
                            args.model,
                            model_features_dict,
