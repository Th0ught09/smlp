...
if [0, 9] <= list(map(int, sns.__version__.split("."))) < [0, 10]:
    ...
    def response_distribution_plot(out_dir, y, resp_names, interactive):
        ...
        sns.distplot(
            ...
            hist=True,
            kde=False,
            bins=ceil(max(10, 50 / len(resp_names))),
            color=[cc(c / len(resp_names)) for c, n in enumerate(resp_names)],
        )
        plt.legend(resp_names, loc="upper right")
    ...
    def response_distribution_plot(out_dir, y, resp_names, interactive):
        ...
        for c in y:
            sns.distplot(
                y[c], hist=True, kde=False, bins=ceil(max(10, 50 / len(resp_names)))
            )
        plot("resp-distr", interactive, out_dir, block=False)
else:
    print(
        "seaborn version",
        sns.__version__,
        "not supported; required: 0.9.x or 0.10.x",
        file=sys.stderr,
    )
    sys.exit(1)
...
def plot_true_pred_runtime1(
    y, y_pred, interactive, title, out_prefix=None, log_scale=False
):
...
def plot_true_pred_runtime(
    y, y_pred, interactive, title, out_prefix=None, log_scale=False
):
    try:
        plot_true_pred_runtime1(
            y, y_pred, interactive, title, out_prefix=out_prefix, log_scale=log_scale
        )
