...
z3_solver::z3_solver(const char *logic) : slv(ctx) {
  if (logic)
    slv.set("logic", logic);
}
...
z3::expr z3_solver::interp(const form2 &f, hmap<void *, z3::expr> &m) {
  return f.match(
      ...
      [&](const lneg2 &n) { return !interp(n.arg, m); });
}
  ...
  return e.match(
      ...
      [&](const cnst2 &c) {
        return c.value.match(
            ...
            [&](const A &) -> z3::expr {
              MDIE(mod_z3, 5, "algebraic real constants are unsupported\n");
            });
      },
      [&](const ite2 &i) {
...
static cnst2 parse_z3_cnst(const str &id, const z3::expr &e) {
  ...
  if (str num; e.is_numeral(num)) {
    ...
    c.value = A({to_Q(cl.value), to_Q(cu.value)}, "x", upoly(move(cc)), i);
  } else {
    std::stringstream ss;
    ss << e;
    MDIE(mod_z3, 3,
         "expected numeral or algebraic "
...
result z3_solver::check() {
  ...
  timnew.it_value.tv_usec +=
      100000; /* 100ms "ought to be enough for every CPU" */
  ...
  case z3::unsat:
    return unsat{};
  case z3::unknown:
    return unknown{slv.reason_unknown()};
  }
  unreachable();
}
