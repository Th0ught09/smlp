...
class SmlpTerms:
    def __init__(self):
        ...
        self._ast_operators_smlp_map = {
            ast.Add: self.smlp_add,
            ast.Sub: self.smlp_sub,
            ast.Mult: self.smlp_mult,
            ast.Div: self.smlp_div,
            ast.Pow: self.smlp_pow,
            ast.BitXor: op.xor,
            ast.USub: op.neg,
            ast.Eq: self.smlp_eq,
            ast.NotEq: self.smlp_ne,
            ast.Lt: self.smlp_lt,
            ast.LtE: self.smlp_le,
            ast.Gt: self.smlp_gt,
            ast.GtE: self.smlp_ge,
            ast.And: self.smlp_and,
            ast.Or: self.smlp_or,
            ast.Not: self.smlp_not,
    ...
    def smlp_iff(self, form1: smlp.form2, form2: smlp.form2):
        return self.smlp_and(
            self.smlp_implies(form1, form2), self.smlp_implies(form2, form1)
        )
            ...
            if isinstance(node, ast.Num):  # <number>
                ...
                if type(node.op) not in [ast.Div, ast.Pow]:
                    ...
                    if type(node.right) == ast.Constant:
                        if node.right.n == 0:
                            raise Exception(
                                "Division by 0 in parsed expression " + expr
                            )
                        elif not isinstance(node.right.n, int):
                            raise Exception(
                                "Division in parsed expression is only supported for integer constants; got "
                                + expr
                            )
                        else:
                        ...
                        raise Exception(
                            "Opreator "
                            + str(self.ast_operators_smlp_map[type(node.op)])
                            + " with non-constant demominator within "
                            + str(expr)
                            + " is not supported in ast_expr_to_term"
                        )
                ...
                raise Exception(
                    "Unsupported comstant "
                    + str(node.n)
                    + " in funtion ast_expr_to_term"
                )
    ...
    def ground_smlp_expr_to_value(
        ...
        if isinstance(self.smlp_cnst(smlp_const), smlp.libsmlp.A) or isinstance(
            ...
            if approximate:
                ...
                try:
                    if (
                        self.smlp_cnst(smlp_const).denominator is not None
                        and self.smlp_cnst(smlp_const).numerator is not None
                    ):
                        val = Fraction(
                            self.smlp_cnst(smlp_const).numerator,
                            self.smlp_cnst(smlp_const).denominator,
                        )
            ...
            raise Exception(
                "Failed to compute value for smlp expression "
                + str(expr)
                + " of type "
                + str(type(expr))
            )
...
class TreeTerms:
    ...
    def tree_encoding_flat(self, algo):
        return (
            self._tree_encoding == self._tree_encodings.flat.name
    ...
    def tree_encoding_branched(self, algo):
        return (
            self._tree_encoding == self._tree_encodings.branched.name
            and algo in self.supported_algos
        )
    ...
    def tree_encoding_nested(self, algo):
        return (
            self._tree_encoding == self._tree_encodings.nested.name
            and algo in self.supported_algos
        )
    ...
    def _tree_model_id(self, algo: str, tree_number: int, resp_name=None):
        ...
        res1 = (
            "_".join([algo, "tree", str(tree_number)])
        ...
        if isinstance(rule, dict):
            ...
            if len(rule["antecedent"]) == 0:
                return (
                    " and ".join(
                        [
                            "(" + " ".join([str(e) for e in list(tup)]) + ")"
                            for tup in consequent_list
                        ]
                    )
    ...
    def trees_to_rules(
        ...
        if rules_filename is not None:
            ...
            self._smlp_terms_logger.info(
                "Writing tree rules into file " + rules_filename
            )
        ...
        for indx, tree_est in enumerate(tree_estimators):
            ...
            rules = self._get_abstract_rules(
                tree_est, feature_names, response_names, class_names
            )
    ...
    def _rule_triplet_to_term(self, p):
        ...
        return (
            self.instSmlpTerms._ast_operators_smlp_map[
                self._inequality_ops_ast_dict[p[1]]
            ]
    ...
    def rules_to_term(
        ...
        def rule_to_form(rule, tree_number):
            ...
            for resp, (ant, val) in res_dict.items():
                resp_rhs = self.instSmlpTerms.smlp_eq(
                    self.instSmlpTerms.smlp_var(self._tree_resp_id(tree_number, resp)),
                    val,
                )
                rhs = (
                    resp_rhs
                    if rhs == self.instSmlpTerms.smlp_true
                    else self.instSmlpTerms.smlp_and(rhs, resp_rhs)
                )
            ...
            if self.tree_encoding_flat(algo):
                ...
                for resp, (ant_term, con_term) in rule_dict.items():
                    if i == 0:
                        ...
                        rules_dict[resp] = (
                            con_term  # self.instSmlpTerms.smlp_ite(ant_term, con_term, smlp.Var('SMLP_UNDEFINED'))
                        )
                    else:
                        rules_dict[resp] = self.instSmlpTerms.smlp_ite(
                            ant_term, con_term, rules_dict[resp]
                        )
    ...
    def tree_model_to_term(self, tree_model, algo, feat_names, resp_names):
        if algo in ["dt_caret", "dt_sklearn"]:
            ...
            raise Exception(
                ...
                + str(algo)
                + " is currently not supported in smlp_opt"
            )
        trees = self.trees_to_rules(
            tree_estimators, feat_names, resp_names, None, False, None
        )
        ...
        if self._compress_rules:
            ...
            unique_conj_befor = count_occurrences(
                ant_reduction_stats["before"]
            )  # sum(list(set(ant_reduction_stats['before'])))
            unique_conj_after = count_occurrences(
                ant_reduction_stats["after"]
            )  # sum(list(set(ant_reduction_stats['after'])))
            ...
            self._smlp_terms_logger.info(
                "Tree rules (branches) antecedent compression statistics for response(s) {}:".format(
                    ",".join(resp_names)
                )
                + "\n\ttrees count in the model   "
                + str(trees_count)
                + "\n\ttree branches/rules count  "
                + str(branches_count)
                + "\n\tantecedent lengths before  "
                + str(ant_len_befor)
                + "\n\tantecedent lengths after   "
                + str(ant_len_after)
                + "\n\tbranch length counts before "
                + str(unique_conj_befor)
                + "\n\tbranch length counts after  "
                + str(unique_conj_after)
                + "\n\ttree max depth before      "
                + str(tree_max_depth_befor)
                + "\n\ttree max depth after       "
        ...
        for j, tree_rules in enumerate(trees):
            ...
            if self.tree_encoding_flat(algo):
                ...
                flat_model_formulas = (
                    flat_model_formulas
                    + tree_term_dict_dict[self._tree_id(j)][
                        self._tree_model_id(algo, j)
                    ]
                )
                if j == number_of_trees - 1:
                    tree_model_term_dict[self._tree_model_id(algo, None, curr_resp)] = (
                        flat_model_formulas
                    )
                    for resp_name in resp_names:
                        sum_term = self.instSmlpTerms.smlp_add_multi(
                            [
                                self.instSmlpTerms.smlp_var(
                                    self._tree_resp_id(i, resp_name)
                                )
                                for i in range(number_of_trees)
                            ]
                        )
                        mean_term = self.instSmlpTerms.smlp_mult(
                            self.instSmlpTerms.smlp_cnst(
                                self.instSmlpTerms.smlp_q(1)
                                / self.instSmlpTerms.smlp_q(int(number_of_trees))
                            ),
                            sum_term,
                        )
                ...
                for resp_name in resp_names:
                    if j == 0:
                        tree_model_term_dict[resp_name] = tree_term_dict_dict[
                            self._tree_id(j)
                        ][resp_name]
                    else:
                        tree_model_term_dict[resp_name] = self.instSmlpTerms.smlp_add(
                            tree_model_term_dict[resp_name],
                            tree_term_dict_dict[self._tree_id(j)][resp_name],
                        )
                        if (
                            j == number_of_trees - 1
                        ):  # the last tree -- compute the mean by dividing the sum on number_of_trees
                            ...
                            tree_model_term_dict[resp_name] = (
                                self.instSmlpTerms.smlp_mult(
                                    self.instSmlpTerms.smlp_cnst(
                                        self.instSmlpTerms.smlp_q(1)
                                        / self.instSmlpTerms.smlp_q(
                                            int(number_of_trees)
                                        )
                                    ),
                                    tree_model_term_dict[resp_name],
    ...
    def tree_models_to_term(self, model, algo, feat_names, resp_names):
        ...
        if isinstance(model, dict):
            ...
            for resp_name in resp_names:
                ...
                tree_model_term_dict[resp_name] = self.tree_model_to_term(
                    tree_model, algo, feat_names, [resp_name]
                )[resp_name]
        else:
...
class PolyTerms:  # (SmlpTerms):
    ...
    def poly_model_to_term(
        self, model_feat_names, model_resp_names, coefs, powers, log, formula_filename
    ):
...
class NNKerasTerms:  # (SmlpTerms):
    ...
    def nnkeras_encoding_layered(self, algo="nn_keras"):
        return (
            self._nnet_encoding == self._nnkeras_encodings.layered.name
            and algo == "nn_keras"
        )

    def nnkeras_encoding_nested(self, algo="nn_keras"):
        return (
            self._nnet_encoding == self._nnkeras_encodings.nested.name
            and algo == "nn_keras"
        )
    ...
    def _nn_dense_layer_terms(
        ...
        print(layer_weights)
        print(layer_biases)
        print(last_layer_terms)
        curr_layer_terms = [
            self._nn_activation_term(
                activation_func,
                self._nn_dense_layer_node_term(
                    last_layer_terms, layer_weights[i], layer_biases[i]
                ),
            )
    ...
    def get_nn_keras_model_type(self, model):
        ...
        if self._nn_keras_is_sequential(model):
            ...
            raise Exception(
                "Unsupported Keras NN type (neither sequential nor functional)"
            )
        return model_type
    ...
    def nn_keras_model_to_term_nested(
        self, model, model_feat_names, model_resp_names, feat_names, resp_names
    ):
    ...
    def _nn_dense_layer_node_formula(
        ...
        layer_term = self._nn_activation_term(
            activation_func,
            self._nn_dense_layer_node_term(
    ...
    def _nn_dense_layer_formulas_layered(
        self,
        layer: int,
        last_layer_vars,
        curr_layer_vars,
        ...
        curr_layer_forms = [
            self._nn_dense_layer_node_formula(
                curr_layer_vars[i],
                last_layer_vars,
                layer_weights[i],
                layer_biases[i],
    ...
    def nn_keras_model_to_term_layered(
        self, model, model_feat_names, model_resp_names, feat_names, resp_names
    ):
        # from pprint import pprint
        # import inspect
        ...
        model_name = (
            "nn_keras_model"
            if resp_name is None
            else "nn_keras_model_" + str(resp_name)
        )
        ...
        for l, layer in enumerate(model.layers):
            ...
            if l == 0:
                ...
                if self.nn_keras_layer_is_input(model, prev_layer):
                    ...
                    previous_layer_nodes = [
                        self._nn_keras_node_name(resp_name, prev_layer_number, node)
            ...
            curr_layer_forms = self._nn_dense_layer_formulas_layered(
                ...
                biases.transpose(),
                layer_activation,
            )  # , is_output_layer, self._nn_keras_is_functional(model)
    ...
    def nn_keras_model_to_term(
        self, model, model_feat_names, model_resp_names, feat_names, resp_names
    ):
        if self.nnkeras_encoding_layered():
            model_term_dict = self.nn_keras_model_to_term_layered(
                model, model_feat_names, model_resp_names, feat_names, resp_names
            )
        elif (
            self.nnkeras_encoding_nested()
        ):  # self._nnet_encoding == self._nnkeras_encodings.NESTED:
            model_term_dict = self.nn_keras_model_to_term_nested(
                model, model_feat_names, model_resp_names, feat_names, resp_names
            )
        else:
            raise Exception(
                "Unexpected NN Keras encoding mode to solver "
                + str(self._nnet_encoding)
            )
...
class ScalerTerms(SmlpTerms):
    ...
    def feature_scaler_to_term(
        ...
        if orig_min == orig_max:
            return self.smlp_cnst(
                0
            )  # smlp.Cnst(0) # same as returning smlp.Cnst(smlp.Q(0))
        else:
    ...
    def feature_scaler_terms(self, data_bounds, feat_names):
        return dict(
            [
                (
                    self._scaled_name(feat),
                    self.feature_scaler_to_term(
                        feat,
                        self._scaled_name(feat),
                        data_bounds[feat]["min"],
                        data_bounds[feat]["max"],
                    ),
                )
...
class ModelTerms(ScalerTerms):
    def __init__(self):
        ...
        self.model_term_params_dict = {
            "compress_rules": {
                "abbr": "compress_rules",
                "default": str(self._DEF_COMPRESS_RULES),
                "type": str_to_bool,
            ...
            "simplify_terms": {
                "abbr": "simplify_terms",
                "default": str(self._DEF_SIMPLIFY_TERMS),
                "type": str_to_bool,
            ...
            "tree_encoding": {
                "abbr": "tree_encoding",
                "default": str(self._DEF_TREE_ENCODING),
                "type": str,
            ...
            "nnet_encoding": {
                "abbr": "nnet_encoding",
                "default": str(self._DEF_NNET_ENCODING),
                "type": str,
                ...
                + "representing the function for that response [default {}]".format(
                    str(self._DEF_NNET_ENCODING)
                ),
            },
    ...
    def smlp_model_term_file(self, resp: str, full: bool):
        ...
        filename_suffix = (
            "smlp_full_model_term.json" if full else "smlp_model_term.json"
        )
        if resp is None:
    ...
    def _compute_pure_model_terms(
        self, algo, model, model_feat_names, model_resp_names, feat_names, resp_names
    ):
        # assert not isinstance(model, dict)
        if algo == "nn_keras":
            model_term_dict = self._nnKerasTermsInst.nn_keras_model_to_term(
                model, model_feat_names, model_resp_names, feat_names, resp_names
            )
        elif algo == "poly_sklearn":
            model_term_dict = self._polyTermsInst.poly_model_to_term(
                model_feat_names,
                model_resp_names,
                model[0].coef_,
                model[1].powers_,
            ...
            model_term_dict = self._treeTermsInst.tree_models_to_term(
                model, algo, model_feat_names, model_resp_names
            )
        else:
            raise Exception(
                "Algo "
    ...
    def _compute_model_terms_dict(
        ...
        # compute model terms
        model_feat_names = (
            [self._scaled_name(feat) for feat in feat_names]
            if feat_were_scaled
            else feat_names
        )  # ._scalerTermsInst
        model_resp_names = (
            [self._scaled_name(resp) for resp in resp_names]
            if resp_were_scaled
            else resp_names
        )  # ._scalerTermsInst
        ...
        nn_keras_layered_encoding = self._nnKerasTermsInst.nnkeras_encoding_layered(
            algo
        )
        ...
        if (
            resp_were_scaled
            and not tree_flat_encoding
            and not nn_keras_layered_encoding
            and not tree_branched_encoding
        ):
            responses_unscaler_terms_dict = self.feature_unscaler_terms(
            ...
            for resp_name, resp_term in responses_unscaler_terms_dict.items():
                ...
                responses_unscaler_terms_dict[resp_name] = self.smlp_cnst_fold(
                    ...
                    {
                        self._scaled_name(resp_name): model_term_dict[
                            self._scaled_name(resp_name)
                        ]
                    },
                )
        ...
        if resp_were_scaled and tree_flat_encoding:
            ...
            for j, resp in enumerate(resp_names):
                ...
                for i in range(tree_count):
                    ...
                    resp_names_numbered_trees.append(
            ...
            model_full_term_dict_new[new_key] = [
                self.smlp_cnst_fold(
                    rule_formula,
                    responses_scaler_terms_dict
                    | responses_scaler_terms_dict_numbered_trees,
                )
    ...
    def compute_models_terms_dict(
        ...
        if isinstance(model_or_model_dict, dict):
            ...
            for i, resp_name in enumerate(model_or_model_dict.keys()):
                ...
                curr_model_full_term_dict = self._compute_model_terms_dict(
                    algo,
                    model_or_model_dict[resp_name],
                    feat_names,
                    [resp_name],
                    data_bounds,
                    data_scaler,
            ...
            models_full_terms_dict = self._compute_model_terms_dict(
                algo,
                model_or_model_dict,
                feat_names,
                resp_names,
    ...
    def compute_objectives_terms(self, objv_names, objv_exprs, objv_bounds, scale_objv):
        ...
        orig_objv_terms_dict = dict(
            [
                (objv_name, self.ast_expr_to_term(objv_expr))
    ...
    def compute_stability_formula_theta(
        ...
        for var, radii in radii_dict_local.items():
            ...
            if radii["rad-abs"] is not None:
                ...
                # if (
                #     delta_rel is not None
                # ):  # radius for a lemma -- cex holds values of candidate counter-example
                # rad_term = rad_term * abs(var_term)
                # radius for excluding a candidate -- cex holds values of the candidate
                rad_term = rad_term * abs(cex[var])
            elif delta_dict is not None:
                raise exception(
                    "When delta dictionary is provided, either absolute or relative radius must be specified"
                )
            ...
            if mn is not None and mx is not None:
                if self._declare_domain_interface_only:
                    if self._encode_input_range_as_disjunction:
                        rng = self.smlp_or_multi(
                            [
                                self.smlp_eq(self.smlp_var(v), self.smlp_cnst(i))
                                for i in range(mn, mx - 1)
                            ]
                        )
                    else:
                        rng = self.smlp_and(
                            self.smlp_var(v) >= self.smlp_cnst(mn),
                            self.smlp_var(v) <= self.smlp_cnst(mx),
                        )
    ...
    def compute_input_ranges_formula_alpha_eta(self, alpha_vs_eta, model_inputs):
        ...
        for v, b in alpha_or_eta_ranges_dict.items():
            ...
            if mn is not None and mx is not None:
                if self._declare_domain_interface_only:
                    if (
                        self._encode_input_range_as_disjunction
                        and alpha_vs_eta == "alpha"
                        and v in self._specInst.get_spec_inputs
                    ):
                        rng = self.smlp_or_multi(
                            [
                                self.smlp_eq(self.smlp_var(v), self.smlp_cnst(i))
    ...
    def var_domain(self, var, spec_domain_dict):
        ...
        if (
            spec_domain_dict[var][self._specInst.get_spec_domain_type_tag]
            == self._specInst.get_spec_integer_tag
        ):
            if self._declare_integer_as_real_with_grid and not interval_has_none:
                var_component = smlp.component(
                    self.smlp_real, grid=list(range(interval[0], interval[1] + 1))
                )
            # assert False
            ...
            spec_domain_dict[var][self._specInst.get_spec_domain_type_tag]
            == self._specInst.get_spec_real_tag
        ):
            if self._declare_domain_interface_only or interval_has_none:
    ...
    def create_model_exploration_base_components(
        self,
        syst_expr_dict: dict,
        algo,
        model,
        ...
        interface_consistent = self.check_alpha_eta_consistency(
            domain_features, None, alpha, eta, "ALL"
        )
        ...
        if self._nnKerasTermsInst.nnkeras_encoding_layered(algo):
            ...
            def declare_iternal_node_vars(model, resp_name, resp_names):
                ...
                for l, layer in enumerate(model.layers):
                    ...
                    is_output_layer = self._nnKerasTermsInst.nn_keras_layer_is_output(
                        model, l, resp_names
                    )
        ...
        if algo == "system":
            if syst_expr_dict is None:
                raise Exception(
                    'System must be specified when model training algorithm is "system"'
                )
        ...
        model_consistent = self.check_alpha_eta_consistency(
            domain, model_full_term_dict, alpha, eta, "ALL"
        )
    ...
    def create_model_exploration_instance_from_smlp_components(
        self, domain, model_full_term_dict, incremental, logic
    ):
    ...
    def smlp_solver_check(self, solver, call_name: str, lemma_precision: int = 0):
        ...
        if call_name == "interface_consistency":
            if self._trace_anonymize:
                interface_column_names = (
                    list(anonym_interface_dict["knobs"].values())
                    + list(anonym_interface_dict["inputs"].values())
                    + list(anonym_interface_dict["outputs"].values())
                )
            else:
                interface_column_names = (
                    list(anonym_interface_dict["knobs"].keys())
                    + list(anonym_interface_dict["inputs"].keys())
                    + list(anonym_interface_dict["outputs"].keys())
                )
            if self._trace_runtime == 0:
                self._smlp_terms_tracer.info(
                    ",".join(["stage", "solver"] + interface_column_names)
                )
            else:
                self._smlp_terms_tracer.info(
                    ",".join(["stage", "solver", "runtime"] + interface_column_names)
                )
        ...
        if status == "sat":
            ...
            for k in sat_model.keys():
                if k in anonym_interface_dict["knobs"].keys():
                    name = (
                        k
                        if not self._trace_anonymize
                        else anonym_interface_dict["knobs"][k]
                    )
                elif k in anonym_interface_dict["inputs"].keys():
                    name = (
                        k
                        if not self._trace_anonymize
                        else anonym_interface_dict["inputs"][k]
                    )
                elif k in anonym_interface_dict["outputs"].keys():
                    name = (
                        k
                        if not self._trace_anonymize
                        else anonym_interface_dict["outputs"][k]
                    )
                else:
            ...
            if self._trace_anonymize:
                knob_values = [
                    str(assignment[e])
                    for e in list(anonym_interface_dict["knobs"].values())
                    if e in assignment.keys()
                ]
                input_values = [
                    str(assignment[e])
                    for e in list(anonym_interface_dict["inputs"].values())
                    if e in assignment.keys()
                ]
                output_values = [
                    str(assignment[e])
                    for e in list(anonym_interface_dict["outputs"].values())
                    if e in assignment.keys()
                ]
                if approx_lemmas:
                    knob_values_approx = [
                        str(assignment_approx[e])
                        for e in list(anonym_interface_dict["knobs"].values())
                        if e in assignment.keys()
                    ]
                    input_values_approx = [
                ...
                knob_values = [
                    str(assignment[e])
                    for e in list(anonym_interface_dict["knobs"].keys())
                    if e in assignment.keys()
                ]
                input_values = [
                    str(assignment[e])
                    for e in list(anonym_interface_dict["inputs"].keys())
                    if e in assignment.keys()
                ]
                output_values = [
                    str(assignment[e])
                    for e in list(anonym_interface_dict["outputs"].keys())
                    if e in assignment.keys()
                ]
                if approx_lemmas:
                    knob_values_approx = [
        ...
        if self._trace_runtime == 0:
            self._smlp_terms_tracer.info(
                ",".join(
                    [call_name, status] + knob_values + input_values + output_values
                )
            )
            if approx_lemmas and status == "sat":
                self._smlp_terms_tracer.info(
                    ",".join(
                        [call_name + "_approx", status]
                        + knob_values_approx
                        + input_values_approx
                        + output_values_approx
                    )
            ...
            if approx_lemmas and status == "sat":
                self._smlp_terms_tracer.info(
                    ",".join(
                        [call_name + "_approx", status, str(elapsed)]
                        + knob_values_approx
                        + input_values_approx
                        + output_values_approx
                    )
    ...
    def get_solver_model(self, res):
        if self.solver_status_sat(res):
            reduced_model = dict(
                ...
                if k in self._specInst.get_spec_interface
            )
    ...
    def get_solver_knobs_model(self, res):
        if self.solver_status_sat(res):
            reduced_model = dict(
                (k, v)
                for k, v in res.model.items()
                if k in self._specInst.get_spec_knobs
            )
            return reduced_model
        else:
            return None
    ...
    def get_solver_resps_model(self, res):
        if self.solver_status_sat(res):
            reduced_model = dict(
                (k, v)
                for k, v in res.model.items()
                if k in self._specInst.get_spec_responses
            )
            return reduced_model
        else:
            return None
    ...
    def check_alpha_eta_consistency(
        self,
        domain: smlp.domain,
        ...
        res = self.smlp_solver_check(
            ...
            if model_full_term_dict is None
        ...
        if isinstance(res, smlp.sat):
            self._smlp_terms_logger.info(
                consistency_type + " interface constraints are consistent"
            )
            interface_consistent = True
        elif isinstance(res, smlp.unsat):
            self._smlp_terms_logger.info(
                consistency_type + " interface constraints are inconsistent"
            )
